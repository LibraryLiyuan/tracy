...
无人值守控制可以传入 `-x <stop-file>`。采集器在主循环中检测到该路径
存在后，执行与首次 Ctrl+C/`-s` 到期完全相同的主动 Disconnect 和协议
排空；它不会删除、改写或解释文件内容。stop-file 必须与 snapshot/journal
输出路径不同，调用方必须在启动前保证 marker 不存在。这个接口用于先让
Client 在被测进程仍存活时排空定义和事件尾部，再结束被测进程，不能用
强杀 capture 或游戏替代。
