...
的修复。背压指标、Client 队列硬上限、自动转换、完整离线符号化和 GUI
支持仍按本文件的触发条件暂缓。
...
8. 转换器和 Query 按相同的延迟符号扩展模式、录制器排空完成条件和
   查询窗口重放，继续进行严格逐字节校验；每条预期服务器记录有 10 秒
   截止时间，缺失或发散会主动关闭本地 socket 并显式失败。
...
### 4.5 已实现有限符号扩展延迟，完整离线符号化仍暂缓

本次事故修复已经实现一个受限版本：`ProtocolOnly` 收到
`CallstackFrame` 响应后，保留 Client 返回的函数名、文件和行号，但不再
继续请求每个子帧的 `ServerQuerySymbol`、符号机器码，也不再用 Capstone
反汇编结果递归查询更多指令地址。v3/v4 重放使用相同模式，传统 Full
实时 Worker 的默认行为不变。

这不是完整离线符号化。最终 trace 仍依赖录制期间 Client 返回的调用栈
帧信息；当前也没有在转换阶段读取 PDB/DWARF 来补回 symbol code 和
disassembly 域。
...
因此进一步演进仍应采用混合 `DeferredSymbols` 模式，而不是完全取消
在线定义：
...
当前 v4 行为相当于固定启用了上面的有限子集，但尚未提供这两个命令行
模式。完整 `deferred-symbols` 仍需要协议/格式扩展、模块身份和外部符号
根配置，以及与严格模式的 Zone、地址、模块、符号和 GUI 结果 A/B；不应
直接修改现有 Full Worker 默认行为。
...
- 保持 v4 `BeginDrain`、原生 Client Disconnect、有限符号扩展延迟和
  记录查询窗口的严格字节重放作为正确性基准；
- 保留第二次 `Ctrl+C` 强制停止和不完整但可恢复前缀作为异常退出路径；
- 不立即扩大 Client 串行队列或改变 Tracy 原始协议语义；
- 不立即实施自动转换、完整离线符号化或 GUI 原生增量；
- 两次 5 秒和一次 10 秒真实高细节录制均已正常完成，10 秒录制还覆盖了
  进行中 revision 查询；若再次出现性能、资源或结束延迟问题，按触发
  条件和新增指标定向改进。
