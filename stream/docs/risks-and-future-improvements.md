...
记录日期：2026-07-24；正式验证补充日期：2026-07-26。

当前 `.tracy-stream` 功能已经完成格式、写盘、恢复、严格重放、
`ProtocolOnly`、直接 Query/MCP 访问以及真实 Godot/TPS 验收。原计划是
等实际问题出现后再处理；随后“录制 5 秒”真实触发了文件持续增长到
15.89 GiB 且无法退出的问题，因此本轮已经实施一组只针对停止/排空路径
的修复。2026-07-26 的正式验证又加入了排空无进展看门狗、并发定义查询
重放校验和严格参数解析，并验证了 Tracy 官方离线符号解析流程。背压指标、
Client 队列硬上限、自动转换和 GUI 支持仍按本文件的触发条件暂缓。

这份文件同时登记已经被真实事故触发并完成的修复，以及尚未实施的后续
方案。各节会明确标注当前状态。
...
原始 5 秒 Godot/TPS 高细节录制在用户手动终止前达到
17,063,103,979 字节（15.89 GiB）。`BeginDrain` 标记约位于 70.3 MiB，
但标记后仍写入 505,346 条记录，其中客户端 payload 约 15.42 GiB，
服务器 payload 约 86.95 MiB。这证明当时并非只处理一个有限的定义尾部：
游戏仍在运行，旧的“仅服务器进入 drain”逻辑继续接收普通事件，排空
目标一边处理一边被新增数据扩大。

排查还发现第二个独立问题。加入原生 Disconnect 后，
`TRACY_ON_DEMAND` Client 的 `HandleDisconnect()` 会在循环中调用
`ClearQueues()`；Symbol Worker 异步生成的调用栈定义响应可能刚进入普通
队列就被清除。一次测试幸运地在约 30 ms 结束，另一次则在最后 117 个
调用栈查询后等待约 175 秒，说明这是时序竞争而不是稳定行为。

最终实时 revision 验证还暴露了第三个离线问题：Full Worker 重放已经
逐字节匹配此前所有服务器记录，却因为保留了 ProtocolOnly 不参与结束
判定的一项 pending 状态而没有生成最后的 `ServerQueryTerminate`。
同时，原有 Socket `Read` 的 timeout 只用于周期轮询，不代表总体截止；
verifier 缺少响应时会无限重试，进一步把明确的协议缺失表现成“转换卡住”。

#### 当前 v4 排空行为

现在的停止路径为：

1. 在 `SessionBegin` v2 中声明 ProtocolOnly 的延迟符号扩展模式，使
   尚未结束、还没有排空标记的实时 revision 也能选择正确重放行为；
2. 等待停止时已经发起的网络读取完成；
3. 写入五字节 v4 `BeginDrain` 本地控制记录：版本号 `4` 加实时连接
   使用的服务器查询窗口；
4. 发送 Tracy 原生 `ServerQueryDisconnect`，使 on-demand Client 停止
   接收新的普通生产者事件；
5. Client 继续序列化停止时已经存在的普通事件尾部（其大小没有 Client
   硬上限），并让 Symbol Worker 完成异步定义响应，不再用
   `ClearQueues()` 丢弃这些响应；
6. `ProtocolOnly` 保留调用栈帧已有的函数名、文件和行号，但不再递归
   请求可选的 `SymbolInformation`、符号机器码和 Capstone 反汇编地址；
7. 必要 pending、queued 和 in-flight 定义归零后，发送普通
   `ServerQueryTerminate` 并写入完整 `SessionEnd`；
8. 转换器和 Query 按相同的延迟符号扩展模式、录制器排空完成条件和
   查询窗口重放。握手、断开、终止和数据分片继续逐包有序校验；仅在
   连续的独立定义查询批次内按完整 payload 多重集校验，接受线程调度造成
   的顺序互换，但仍拒绝缺包、改字节或跨控制边界重排；
9. 每条预期服务器记录有 10 秒本地读取截止。ProtocolOnly 排空还默认有
   30 秒无进展看门狗，同时观察已提交字节、双向协议字节、事件和定义；
   超时以退出码 124 保留不完整但可恢复的前缀，不写伪造的 `SessionEnd`。
   `-d 0..3600` 可调整阈值，`0` 明确禁用。

旧控制记录 v1、v2、v3 仍可读取；新录制统一使用 v4。第二次按
`Ctrl+C` 会以退出码 130 强制停止异常排空，不写伪造的完整终止记录，
但已经提交的 journal 前缀仍可恢复和转换。

#### 修复后的真实 Godot/TPS 结果

相同 Godot `tracy_ai_full`、D3D12、完整内存追踪和 62 层调用栈配置下，
连续两次 5 秒录制均正常完成：

| 录制文件 | 有效采集 | 排空 | journal | 排空后客户端 payload |
| --- | ---: | ---: | ---: | ---: |
| `TPS-protocol-only-20260724-105424.tracy-stream` | 5.05 秒 | 2.32 秒 | 75,390,903 字节 | 757,074 字节 |
| `TPS-protocol-only-20260724-105536.tracy-stream` | 5.03 秒 | 2.88 秒 | 83,241,470 字节 | 939,616 字节 |
| `TPS-live-v2-20260724-112410.tracy-stream` | 10.06 秒 | 8.02 秒 | 79,516,492 字节 | 1,365,769 字节 |

两次均使用查询窗口 5,037，写入 `CaptureComplete` 的 `SessionEnd`，通过
严格转换和 Query doctor。第二次录制的 stream 与转换后 trace 还通过了
全部 12 个 MCP 工具、三类 compare、异步 validate 和资源读取；两者的
377 帧、29,627 个 CPU Zone、544,372 个调用栈 payload、1,789,839 个
内存事件、28,944 个锁事件等语义计数一致。

最终 10 秒 v2 录制还在 journal 只有 11,973,843 字节、未出现排空标记时
通过了实时 Query doctor；完成后严格匹配 4,400 条客户端记录和 8,882 条
服务器记录，生成 30,726,331 字节 `.tracy`，并再次通过全部 12 个 MCP
工具。

当前修复消除了“运行中的游戏持续为排空增加普通事件”“异步定义响应被清
队列丢失”“Full 重放使用不同完成条件”和“verifier 无限等待”四个已经
复现的根因。它不承诺任何工作负载都在固定时间内结束：断开前已有队列、
必要动态定义、Client 异常或磁盘问题仍可能造成较长尾部，因此强制停止
和可恢复前缀仍有必要。

#### 2026-07-26 正式验证补充

在线 DbgHelp 符号模式的三次 5 秒高细节短录中，一次完整结束，两次在
最后的定义查询停止前进后由默认 30 秒看门狗以退出码 124 有界终止；两份
journal 均保留可恢复前缀。60 秒在线录制则完整结束，并在 10、30、50 秒
三个未完成 revision 上通过实时 Query。结论是：看门狗已经消除“无限增长
且永不退出”，但在线符号模式仍不保证每次主动停止都获得完整 SessionEnd。

将游戏启动环境设为 `TRACY_SYMBOL_OFFLINE_RESOLVE=1` 后，三次独立 5 秒
探针全部完整结束，排空分别约 14.59 ms、14.64 ms 和 76.92 ms。随后执行的
最终矩阵包含双写 5 秒、三次 ProtocolOnly 5 秒、ProtocolOnly 60 秒及真实
强停恢复，共 6/6 通过；60 秒录制继续覆盖 10、30、50 秒实时读取。严格
转换、直接 `.tracy-stream` Query、转换后 `.tracy` doctor 和语义计数均
通过。高细节自动化的推荐路径因此改为“游戏端离线符号模式 + 录制后
`tracy-update -r`”，而不是依赖在线 PDB 响应完成排空。

确定性故障注入还在握手后挂起 Godot：3 秒看门狗使采集器以 124 退出，
从挂起到退出 5.284 秒；8,888,459 字节全部位于有效前缀，直接 Query、
严格转换和转换后 doctor 均通过。该测试证明异常 Client 不再导致无界等待。
...
### 4.5 Tracy 官方离线符号化路径已验证

Tracy 0.13.1 已经提供所需能力，不需要新增自定义协议。游戏启动前设置
`TRACY_SYMBOL_OFFLINE_RESOLVE=1` 后，游戏内 Client 保存模块路径和地址
偏移，不在录制期间为每个调用栈帧执行完整 DbgHelp 符号解析。运行时动态
字符串、线程/Fiber 名称、SourceLocation、Callstack ID、模块映射和
Frame Image 等只能从进程获得的数据仍按原协议采集。
...
# 在同一环境中启动游戏，使游戏继承该变量。
tracy-capture.exe -j capture.tracy-stream -s 30 -d 30
tracy-stream-convert.exe -i capture.tracy-stream -o replayed.tracy
tracy-update.exe -r replayed.tracy resolved.tracy
```

变量必须由游戏进程继承，只给独立的 `capture.exe` 设置不会改变游戏内
Client。`tracy-update -r` 在游戏结束后读取匹配的 EXE、DLL 和 PDB；这些
文件若被替换、移动或版本不匹配，离线解析质量会下降，但原始地址和其余
采集数据仍在 trace 中。

最新真实 TPS 样本包含 86,576 个调用栈帧和 7 个映像组，独立
`tracy-update -r` 用时 3.835 秒并生成可通过 doctor 的 `resolved.tracy`。
原始 `.tracy-stream`、严格转换结果和符号补全结果之间的 12 项 MCP 工作流、
三类 compare、源码资源和帧图像资源均通过。该模式现已作为高细节流式
采集的推荐默认工作流。

原在线符号模式仍保留，适合短录、PDB 响应稳定或必须立即得到符号名的
场景；看门狗使其失败有界。无需再设计 `--definition-mode` 或新的模块身份
格式，除非未来要支持 Tracy 官方离线格式之外的符号源。
...
缓冲、优化队列、设置主动终止策略，还是调整离线符号资产和解析流程。

## 6. 当前保留结论

- 保持当前严格、无静默丢失的 `.tracy-stream` 行为；
- 保持 v4 `BeginDrain`、原生 Client Disconnect、有限符号扩展延迟和
  查询窗口重放作为正确性基准；独立定义查询只允许在连续批次内等价重排；
- 保留默认 30 秒无进展看门狗、第二次 `Ctrl+C` 强制停止和不完整但可恢复
  前缀作为异常退出路径；
- 不立即扩大 Client 串行队列或改变 Tracy 原始协议语义；
- 高细节自动化优先使用 `TRACY_SYMBOL_OFFLINE_RESOLVE=1`，结束后显式执行
  `tracy-stream-convert` 和 `tracy-update -r`；
- 不立即实施自动转换或 GUI 原生增量；
- 2026-07-26 最终真实矩阵 6/6 通过，包含 60 秒录制中的三个实时 revision、
  真实强停恢复、严格转换和 MCP 全工具。若再次出现性能、资源或结束延迟
  问题，按触发条件和新增指标定向改进。
...
- [2026-07-26 正式验证报告](formal-validation-20260726.md)
