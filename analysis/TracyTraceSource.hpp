...
public:
    ...
    virtual std::vector<CpuZoneDto> ScanCpuZonesForThread(
        std::string_view threadRef, const ScanRange& range ) const
    {
        ...
        while( result.size() < range.limit )
        {
            ...
            const auto values = ScanCpuZones( sourceRange );
            for( const auto& value : values )
            {
                if( value.threadRef != threadRef ) continue;
    ...
    virtual std::vector<GpuZoneDto> ScanGpuZonesForContext(
        std::string_view contextRef, const ScanRange& range ) const
    {
        ...
        while( result.size() < range.limit )
        {
            ...
            sourceRange.offset = rawOffset;
            sourceRange.limit = Chunk;
            const auto values = ScanGpuZones( sourceRange );
            for( const auto& value : values )
            {
                if( value.contextRef != contextRef ) continue;
                if( matched++ < range.offset ) continue;
                result.emplace_back( value );
                if( result.size() == range.limit ) break;
            }
