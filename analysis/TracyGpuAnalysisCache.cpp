    ...
    const GpuAnalysisSnapshot& snapshot, std::string& error )
{
    ...
    try
    {
        std::filesystem::create_directories( AnalysisIoPath( path.parent_path() ) );
        auto temporary = path; temporary += ".tmp";
        std::ofstream out( AnalysisIoPath( temporary ), std::ios::binary | std::ios::trunc );
...
#undef WS
        ...
        std::error_code ec; std::filesystem::remove( AnalysisIoPath( path ), ec ); ec.clear();
        std::filesystem::rename( AnalysisIoPath( temporary ), AnalysisIoPath( path ), ec );
    ...
    const GpuAnalysisCacheIdentity& identity, std::string& error )
{
    ...
    try
    {
        std::ifstream in( AnalysisIoPath( path ), std::ios::binary );
