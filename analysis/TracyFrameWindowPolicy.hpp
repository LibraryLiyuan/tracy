...
// One full signature at a time; no signature x frame dense matrix. Local Top
// reservoirs are separately bounded per frame/window, thread and metric.
inline std::vector<Signal> Scan( const CandidatePolicyInput& input )
{
    ...
    // Explicit full-series synthetic/legacy adapters can supply a frame root.
    for( const auto& c : input.signatures ) if( c.frameRoot && c.frameSeriesComplete && !timelines.contains( c.frameScope ) )
    ...
    struct TopItem { int64_t cost; size_t context; uint64_t frame; std::string manifestation; nlohmann::json evidence; };
    std::map<TopKey, std::vector<TopItem>> tops;
    std::map<std::pair<size_t, std::string>, Signal> signals;
    ...
    for( size_t ci = 0; ci < input.signatures.size(); ++ci )
    {
        ...
        const auto& c = input.signatures[ci];
        if( !c.frameSeriesComplete ) continue;
        ...
        const auto top = [&]( const std::string& target, const std::string& manifestation,
            ...
            auto& entries = tops[{c.frameScope, c.threadOrQueue, c.metricPreference, target}];
            entries.push_back( { frame.valueNs, ci, frame.frameIndex, manifestation, evidence } );
            std::sort( entries.begin(), entries.end(), [&]( const auto& a, const auto& b ) {
                if( a.cost != b.cost ) return a.cost > b.cost;
                return input.signatures[a.context].signatureId < input.signatures[b.context].signatureId;
            } );
    ...
    for( const auto& [_, entries] : tops ) for( const auto& item : entries )
    {
        const auto existing = signals.find( { item.context, item.manifestation } );
        if( existing == signals.end() )
            select( item.context, item.manifestation,
                item.manifestation.starts_with( "stable_slow:" ) ? "stable_slow_window" :
                item.manifestation == "frame_cost" ? "frame_local_top" : "sustained_pressure_window",
                { item.frame, item.cost, {}, {}, true }, item.evidence );
