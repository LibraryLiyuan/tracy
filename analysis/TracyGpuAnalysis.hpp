...
struct GpuAnalysisBuildControl
{
    ...
    // Query's allocation-only scan normalizes Resource/Allocation/View records
    // without constructing the full Pass/Range snapshot. This intermediate
    // result is not a complete GPU evidence graph.
    bool catalogOnly = false;
    // Optional preservation of generation-scoped Catalog string dictionaries.
