    ...
    const GpuMemoryAttribution* attribution, const GpuAnalysisBudget& budget, const GpuAnalysisBuildControl& control )
{
    ...
    const bool hasPassEvidence = !control.catalogOnly &&
        ( attribution ? !attribution->passes.empty() : !data.gpuReferencePasses.empty() );
    ...
    if( cancelled() ) return out;
    if( !control.catalogOnly ) report( 0.62f, "passes" );
    if( !control.catalogOnly && attribution )
    {
