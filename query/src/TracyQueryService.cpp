...
const std::vector<std::string>& RawQueryMethodRegistry()
{
    static const std::vector<std::string> methods = {
        ...
        "resource.relation.search", "resource.quality",
        "resource.capability", "object.search", "object.get", "asset.search", "asset.get",
        "mesh.get", "texture.get", "resource.lifecycle", "resource.relations",
        "resource.memory_impact", "resource.gpu_usage", "resource.physical.occupants", "resource.io_history",
        "resource.evidence_path", "resource.validation",
...
nlohmann::json RequiredParametersFor( const std::string& method )
{
    ...
    if( ( method.ends_with( ".get" ) && method != "frame.get" && method != "producer.get" && method != "catalog.get" ) || method == "job.dependencies" || method == "job.gfx_chain" || method == "io.chain" || method == "entity.related" || method == "correlation.chain" || method == "timeline.correlated_slice" || method == "zone.cpu.tree" || method == "zone.gpu.tree" || method == "source.lines" || method == "source.raw" ||
        method == "symbol.raw_code" || method == "symbol.disassembly" || method == "frame_image.metadata" || method == "frame_image.resource" || method == "frame_image.raw" ||
        method == "resource.lifecycle" || method == "resource.relations" || method == "resource.memory_impact" ||
        method == "resource.gpu_usage" || method == "resource.physical.occupants" || method == "resource.io_history" || method == "resource.evidence_path" ) required.emplace_back( "ref" );
...
std::string MethodDomain( const std::string& method )
{
    ...
    if( method.rfind( "resource.", 0 ) == 0 || method.rfind( "object.", 0 ) == 0 ||
        method.rfind( "asset.", 0 ) == 0 || method == "mesh.get" || method == "texture.get" ) return "resource";
...
void QueryService::EvictCache( size_t incomingBytes )
{
    while( m_cacheBytes != 0 && ( incomingBytes > m_cacheBudget || m_cacheBytes > m_cacheBudget - incomingBytes ) )
    {
        ...
        for( const auto& [key, entry] : m_gpuCache ) if( entry.value.use_count() == 1 && entry.access < oldestAccess ) { kind = 1; oldestKey = key; oldestAccess = entry.access; }
        for( const auto& [key, entry] : m_memoryCache ) if( entry.value.use_count() == 1 && entry.access < oldestAccess ) { kind = 2; oldestKey = key; oldestAccess = entry.access; }
        for( const auto& [key, entry] : m_resourceGraphCache ) if( entry.value.use_count() == 1 && entry.access < oldestAccess ) { kind = 3; oldestKey = key; oldestAccess = entry.access; }
        if( oldestKey.empty() ) break;
        if( kind == 1 ) { m_cacheBytes -= m_gpuCache.at( oldestKey ).bytes; m_gpuCache.erase( oldestKey ); }
        else if( kind == 2 ) { m_cacheBytes -= m_memoryCache.at( oldestKey ).bytes; m_memoryCache.erase( oldestKey ); }
        else { m_cacheBytes -= m_resourceGraphCache.at( oldestKey ).bytes; m_resourceGraphCache.erase( oldestKey ); }
    }
}
...
void QueryService::EraseTraceCache( const std::string& traceId )
{
    ...
    for( auto it = m_resourceGraphCache.begin(); it != m_resourceGraphCache.end(); )
    {
        if( it->first.rfind( prefix, 0 ) == 0 ) { m_cacheBytes -= it->second.bytes; it = m_resourceGraphCache.erase( it ); } else ++it;
    }
}
...
std::shared_ptr<const QueryService::ResourceGraphIndex> QueryService::CachedResourceGraphIndex( const std::string& traceId,
    const std::shared_ptr<analysis::TraceSource>& source )
{
    const auto key = traceId + "|resource-graph-v2";
    const auto found = m_resourceGraphCache.find( key );
    if( found != m_resourceGraphCache.end() ) { found->second.access = ++m_cacheClock; return found->second.value; }

    auto value = std::make_shared<ResourceGraphIndex>();
    value->assets = source->GetResourceAssets();
    value->assetUpdates = source->GetResourceAssetUpdates();
    value->objectEvents = source->GetUnityObjectEvents();
    value->roots = source->GetNativeRoots();
    value->gfx = source->GetGfxResourceBindings();
    value->parts = source->GetResourceParts();
    value->ranges = source->GetResourceRanges();
    value->contexts = source->GetResourceContexts();
    value->metadata = source->GetResourceMetadata();
    value->relations = source->GetResourceRelations();
    value->bootstrap = source->GetResourceBootstrapStates();
    value->quality = source->GetResourceQuality();

    for( const auto& item : value->assets ) value->assetByEntity[item.entityId] = &item;
    for( const auto& item : value->assetUpdates ) value->updatesByAsset[item.entityId].push_back( &item );
    for( const auto& item : value->objectEvents ) value->objectEventsById[item.objectId].push_back( &item );
    for( const auto& item : value->roots ) value->rootsByObject[item.objectId].push_back( &item );
    for( const auto& item : value->gfx )
    {
        value->gfxById[item.gfxResourceId].push_back( &item );
        value->gfxByIdentity[{ item.gfxResourceId, item.generation, 4 }].push_back( &item );
        value->gfxByTarget[item.targetId].push_back( &item );
    }
    for( const auto& item : value->parts ) value->partsByOwner[item.ownerId].push_back( &item );
    for( const auto& item : value->ranges )
    {
        value->rangesByPart[item.partId].push_back( &item );
        value->rangeHistory[{ item.partId, { item.targetId, item.generation, item.targetKind } }].push_back( &item );
    }
    for( const auto& item : value->metadata ) if( item.entityKind < value->metadataByEntity.size() )
        value->metadataByEntity[item.entityKind][item.entityId].push_back( &item );
    for( const auto& item : value->relations )
    {
        const ResourceEntityKey sourceKey { item.sourceId, item.sourceEpoch, item.sourceKind };
        const ResourceEntityKey targetKey { item.targetId, item.targetEpoch, item.targetKind };
        value->relationsBySource[sourceKey].push_back( &item );
        value->relationsByTarget[targetKey].push_back( &item );
        value->relationHistory[{ sourceKey, targetKey, item.relation }].push_back( &item );
    }

    const auto sortTime = []( auto& map ) {
        for( auto& [mapKey, history] : map ) std::stable_sort( history.begin(), history.end(), []( const auto* lhs, const auto* rhs ) {
            return lhs->timeNs < rhs->timeNs;
        } );
    };
    sortTime( value->updatesByAsset ); sortTime( value->objectEventsById ); sortTime( value->rootsByObject );
    sortTime( value->gfxById ); sortTime( value->gfxByIdentity ); sortTime( value->gfxByTarget );
    sortTime( value->partsByOwner ); sortTime( value->rangesByPart );
    for( auto& byId : value->metadataByEntity ) sortTime( byId );
    sortTime( value->relationsBySource ); sortTime( value->relationsByTarget );
    for( auto& [historyKey, history] : value->relationHistory ) std::stable_sort( history.begin(), history.end(), []( const auto* lhs, const auto* rhs ) {
        return lhs->timeNs != rhs->timeNs ? lhs->timeNs < rhs->timeNs : lhs->revision < rhs->revision;
    } );
    for( auto& [historyKey, history] : value->rangeHistory ) std::stable_sort( history.begin(), history.end(), []( const auto* lhs, const auto* rhs ) {
        return lhs->timeNs != rhs->timeNs ? lhs->timeNs < rhs->timeNs : lhs->revision < rhs->revision;
    } );

    const size_t recordCount = value->assets.size() + value->assetUpdates.size() + value->objectEvents.size() +
        value->roots.size() + value->gfx.size() + value->parts.size() + value->ranges.size() + value->contexts.size() +
        value->metadata.size() + value->relations.size() + value->bootstrap.size() + value->quality.size();
    const size_t bytes = sizeof( *value ) + recordCount * 128 +
        ( value->relationHistory.size() + value->rangeHistory.size() ) * 96;
    EvictCache( bytes );
    if( bytes <= m_cacheBudget && m_cacheBytes <= m_cacheBudget - bytes )
    {
        m_cacheBytes += bytes;
        m_resourceGraphCache.emplace( key, ResourceGraphCacheEntry { value, bytes, ++m_cacheClock } );
    }
    return value;
}
...
json QueryService::Dispatch( const json& id, const std::string& method, const json& params, const std::optional<std::string>& defaultTraceId, std::stop_token stopToken )
{
    ...
    if( method == "system.describe" )
    {
        ...
        data["analysis_cache"] = { { "bytes", Decimal( uint64_t( m_cacheBytes ) ) },
            { "entries", m_gpuCache.size() + m_memoryCache.size() + m_resourceGraphCache.size() },
            { "resource_graph_typed_indexes", m_resourceGraphCache.size() },
            { "policy", "LRU; entries in use are not evicted" } };
    ...
    if( method == "resource.summary" || method == "resource.asset.search" || method == "resource.object.search" ||
        method == "resource.part.search" || method == "resource.range.search" ||
        method == "resource.context.search" || method == "resource.metadata.search" ||
        method == "resource.relation.search" || method == "resource.quality" )
    {
        const auto capabilities = source->GetCapabilities();
        const auto capability = std::find_if( capabilities.begin(), capabilities.end(),
            []( const auto& value ) { return value.domain == "resource"; } );
        uint32_t resourceGraphSchema = 0;
        const auto captureIdentity = CaptureIdentityJson( info() );
        if( captureIdentity.value( "present", false ) && captureIdentity.contains( "identity" ) &&
            captureIdentity["identity"].is_object() && captureIdentity["identity"].contains( "protocol" ) &&
            captureIdentity["identity"]["protocol"].is_object() )
        {
            const auto& protocol = captureIdentity["identity"]["protocol"];
            if( protocol.contains( "resource_graph_schema_version" ) )
            {
                const auto& value = protocol["resource_graph_schema_version"];
                if( value.is_string() )
                {
                    try { resourceGraphSchema = uint32_t( std::stoul( value.get<std::string>() ) ); } catch( ... ) {}
                }
                else if( value.is_number_unsigned() ) resourceGraphSchema = value.get<uint32_t>();
            }
        }
        const bool capabilityPresent = capability != capabilities.end() && capability->present;
        const bool present = capabilityPresent && resourceGraphSchema == 2;
        const std::string reason = present ? "" : resourceGraphSchema == 1 ?
            "resource_graph_unsupported_version" : capability == capabilities.end() ?
            "trace_predates_unity_resource_graph" : capability->reason;
        auto base = [&]() -> json { return {
            { "present", present }, { "resource_graph_schema_version", resourceGraphSchema },
            { "reason", reason }, { "provenance", "exact-binary" }, { "trust", "untrusted_trace_data" }
        }; };
        if( !present )
        {
            auto result = base();
            result["complete"] = false;
            if( method == "resource.asset.search" ) result["assets"] = json::array();
            else if( method == "resource.object.search" ) result["objects"] = json::array();
            else if( method == "resource.part.search" ) result["parts"] = json::array();
            else if( method == "resource.range.search" ) result["ranges"] = json::array();
            else if( method == "resource.context.search" ) result["contexts"] = json::array();
            else if( method == "resource.metadata.search" ) result["metadata"] = json::array();
            else if( method == "resource.relation.search" ) result["relations"] = json::array();
            else if( method == "resource.quality" ) result["quality"] = json::array();
            return Success( id, std::move( result ), trace );
        }

        const auto resourceIndex = CachedResourceGraphIndex( trace.id, source );
        const auto& assets = resourceIndex->assets;
        const auto& assetUpdates = resourceIndex->assetUpdates;
        const auto& objectEvents = resourceIndex->objectEvents;
        const auto& roots = resourceIndex->roots;
        const auto& gfx = resourceIndex->gfx;
        const auto& parts = resourceIndex->parts;
        const auto& ranges = resourceIndex->ranges;
        const auto& contexts = resourceIndex->contexts;
        const auto& metadata = resourceIndex->metadata;
        const auto& relations = resourceIndex->relations;
        const auto& bootstrap = resourceIndex->bootstrap;
        const auto& quality = resourceIndex->quality;
        const bool bootstrapComplete = !bootstrap.empty() && bootstrap.back().phase == 4 && bootstrap.back().remainingCount == 0;
        uint64_t dropped = 0, untracked = 0;
        for( const auto& item : quality )
        {
            if( item.counter == 1 ) dropped += item.value;
            else if( item.counter == 3 ) untracked += item.value;
        }

        if( method == "resource.summary" )
        {
            std::unordered_set<uint64_t> objectIds;
            uint64_t creates=0, updates=0, destroys=0;
            for( const auto& value : objectEvents )
            {
                objectIds.emplace( value.objectId );
                if( value.eventKind == 1 ) creates++; else if( value.eventKind == 2 ) updates++; else if( value.eventKind == 3 ) destroys++;
            }
            auto result = base();
            result["complete"] = bootstrapComplete && dropped == 0;
            result["counts"] = { { "assets", Decimal( assets.size() ) }, { "asset_updates", Decimal( assetUpdates.size() ) },
                { "objects", Decimal( objectIds.size() ) },
                { "object_creates", Decimal( creates ) }, { "object_updates", Decimal( updates ) },
                { "object_destroys", Decimal( destroys ) }, { "native_roots", Decimal( roots.size() ) },
                { "gfx_bind_events", Decimal( gfx.size() ) }, { "parts", Decimal( parts.size() ) },
                { "ranges", Decimal( ranges.size() ) }, { "contexts", Decimal( contexts.size() ) },
                { "metadata", Decimal( metadata.size() ) }, { "relations", Decimal( relations.size() ) },
                { "quality_records", Decimal( quality.size() ) }, { "dropped", Decimal( dropped ) },
                { "untracked", Decimal( untracked ) } };
            result["bootstrap"] = bootstrap.empty() ? json( nullptr ) : json {
                { "phase", bootstrap.back().phase }, { "remaining_count", Decimal( uint64_t( bootstrap.back().remainingCount ) ) },
                { "high_watermark", Decimal( bootstrap.back().highWatermark ) }, { "complete", bootstrapComplete } };
            return Success( id, std::move( result ), trace );
        }

        const auto page = ParsePage( params, method, trace );
        if( method == "resource.asset.search" )
        {
            const auto begin = std::min( page.offset, assets.size() ); const auto end = std::min( begin + page.limit, assets.size() );
            json values = json::array();
            for( size_t index=begin; index<end; index++ )
            {
                const auto& value=assets[index]; json updates=json::array();
                for( const auto& update : assetUpdates ) if( update.entityId == value.entityId ) updates.push_back( {
                    { "field", update.field }, { "value", Decimal( update.value ) },
                    { "text", update.text.empty() ? json( nullptr ) : json( update.text ) },
                    { "revision", update.revision }, { "time_ns", Decimal( update.timeNs ) }, { "flags", update.flags } } );
                values.push_back( { { "ref", value.ref }, { "entity_id", Decimal( value.entityId ) },
                    { "asset_id_high", Decimal( value.idHigh ) }, { "asset_id_low", Decimal( value.idLow ) },
                    { "identity_namespace", value.identityNamespace }, { "asset_kind", value.assetKind },
                    { "flags", value.flags }, { "updates", std::move( updates ) } } );
            }
            auto result=base(); result["complete"]=bootstrapComplete && dropped==0; result["matched_count"]=Decimal( assets.size() ); result["assets"]=std::move(values);
            const auto cursor=NextCursor(page,method,trace,end-begin,end<assets.size()); return Success(id,std::move(result),trace,PageJson(page,end-begin,cursor,false));
        }
        if( method == "resource.object.search" )
        {
            std::unordered_map<uint64_t, json> byId;
            std::vector<uint64_t> order;
            for( const auto& value : objectEvents )
            {
                if( !byId.contains( value.objectId ) )
                {
                    order.emplace_back( value.objectId );
                    byId[value.objectId] = { { "object_id", Decimal( value.objectId ) }, { "created", false },
                        { "destroyed", false }, { "updates", json::array() } };
                }
                auto& object = byId[value.objectId];
                if( value.eventKind == 1 )
                {
                    object["created"] = true; object["create_time_ns"] = Decimal( value.timeNs );
                    object["instance_id"] = value.instanceId; object["native_pointer"] = Decimal( value.nativePointer );
                    object["runtime_type_index"] = value.runtimeTypeIndex; object["flags"] = value.flags;
                }
                else if( value.eventKind == 2 ) object["updates"].push_back( {
                    { "time_ns", Decimal( value.timeNs ) }, { "revision", value.revision }, { "field_mask", value.fieldMask },
                    { "value", Decimal( value.value ) }, { "text", value.text.empty() ? json( nullptr ) : json( value.text ) }, { "flags", value.flags } } );
                else { object["destroyed"] = true; object["destroy_time_ns"] = Decimal( value.timeNs ); }
            }
            const auto begin = std::min( page.offset, order.size() );
            const auto end = std::min( begin + page.limit, order.size() );
            json values = json::array();
            for( size_t index=begin; index<end; index++ ) values.emplace_back( std::move( byId[order[index]] ) );
            auto result = base(); result["complete"] = bootstrapComplete && dropped == 0;
            result["matched_count"] = Decimal( order.size() ); result["objects"] = std::move( values );
            const auto cursor = NextCursor( page, method, trace, end-begin, end < order.size() );
            return Success( id, std::move( result ), trace, PageJson( page, end-begin, cursor, false ) );
        }

        if( method == "resource.relation.search" )
        {
            ...
            for( size_t index=begin; index<end; index++ )
            {
                const auto& value = relations[index];
                values.push_back( { { "ref", value.ref }, { "source_id", Decimal( value.sourceId ) },
                    { "target_id", Decimal( value.targetId ) }, { "time_ns", Decimal( value.timeNs ) },
                    { "thread_ref", value.threadRef }, { "relation_revision", value.revision }, { "source_kind", value.sourceKind },
                    { "target_kind", value.targetKind }, { "relation", value.relation },
                    { "source_epoch", value.sourceEpoch }, { "target_epoch", value.targetEpoch },
                    { "action", uint8_t( value.flags >> 6 ) }, { "end_reason", uint8_t( value.provenance >> 4 ) },
                    { "provenance", uint8_t( value.provenance & 0x0F ) }, { "flags", uint8_t( value.flags & 0x3F ) } } );
            }
            auto result=base(); result["complete"] = bootstrapComplete && dropped == 0;
            result["matched_count"] = Decimal( relations.size() ); result["relations"] = std::move( values );
            const auto cursor=NextCursor( page, method, trace, end-begin, end<relations.size() );
            return Success( id, std::move( result ), trace, PageJson( page, end-begin, cursor, false ) );
        }

        if( method == "resource.part.search" )
        {
            const auto begin=std::min(page.offset,parts.size()); const auto end=std::min(begin+page.limit,parts.size()); json values=json::array();
            for(size_t index=begin; index<end; index++){const auto& value=parts[index]; values.push_back({
                {"ref",value.ref},{"part_id",Decimal(value.partId)},{"owner_id",Decimal(value.ownerId)},
                {"index",value.index},{"semantic_kind",value.semanticKind},{"owner_kind",value.ownerKind},
                {"time_ns",Decimal(value.timeNs)},{"thread_ref",value.threadRef},{"flags",value.flags}});}
            auto result=base(); result["complete"]=bootstrapComplete&&dropped==0; result["matched_count"]=Decimal(parts.size()); result["parts"]=std::move(values);
            const auto cursor=NextCursor(page,method,trace,end-begin,end<parts.size()); return Success(id,std::move(result),trace,PageJson(page,end-begin,cursor,false));
        }

        if( method == "resource.range.search" )
        {
            const auto begin=std::min(page.offset,ranges.size()); const auto end=std::min(begin+page.limit,ranges.size()); json values=json::array();
            for(size_t index=begin; index<end; index++){const auto& value=ranges[index]; values.push_back({
                {"ref",value.ref},{"part_id",Decimal(value.partId)},{"target_id",Decimal(value.targetId)},
                {"byte_offset",Decimal(uint64_t(value.byteOffset))},{"byte_length",Decimal(uint64_t(value.byteLength))},
                {"target_epoch",value.generation},{"range_revision",value.revision},{"time_ns",Decimal(value.timeNs)},
                {"target_kind",value.targetKind},
                {"action",uint8_t(value.flags>>6)},{"flags",uint8_t(value.flags&0x3F)}});}
            auto result=base(); result["complete"]=bootstrapComplete&&dropped==0; result["matched_count"]=Decimal(ranges.size()); result["ranges"]=std::move(values);
            const auto cursor=NextCursor(page,method,trace,end-begin,end<ranges.size()); return Success(id,std::move(result),trace,PageJson(page,end-begin,cursor,false));
        }

        if( method == "resource.context.search" )
        {
            const auto begin=std::min(page.offset,contexts.size()); const auto end=std::min(begin+page.limit,contexts.size()); json values=json::array();
            for(size_t index=begin; index<end; index++){const auto& value=contexts[index]; values.push_back({
                {"ref",value.ref},{"context_id",Decimal(value.contextId)},{"related_id",Decimal(value.relatedId)},
                {"related_kind",value.relatedKind},{"stage",value.stage},{"callsite_id",value.callsiteId},
                {"time_ns",Decimal(value.timeNs)},{"thread_ref",value.threadRef},{"flags",value.flags}});}
            auto result=base(); result["complete"]=bootstrapComplete&&dropped==0; result["matched_count"]=Decimal(contexts.size()); result["contexts"]=std::move(values);
            const auto cursor=NextCursor(page,method,trace,end-begin,end<contexts.size()); return Success(id,std::move(result),trace,PageJson(page,end-begin,cursor,false));
        }

        if( method == "resource.metadata.search" )
        {
            const auto begin=std::min(page.offset,metadata.size()); const auto end=std::min(begin+page.limit,metadata.size()); json values=json::array();
            for(size_t index=begin; index<end; index++){const auto& value=metadata[index]; values.push_back({
                {"ref",value.ref},{"entity_id",Decimal(value.entityId)},{"entity_kind",value.entityKind},{"key",value.key},
                {"value_kind",value.valueKind},{"value",Decimal(value.value)},
                {"text",value.text.empty()?json(nullptr):json(value.text)},{"revision",value.revision},
                {"time_ns",Decimal(value.timeNs)},{"thread_ref",value.threadRef}});}
            auto result=base(); result["complete"]=bootstrapComplete&&dropped==0; result["matched_count"]=Decimal(metadata.size()); result["metadata"]=std::move(values);
            const auto cursor=NextCursor(page,method,trace,end-begin,end<metadata.size()); return Success(id,std::move(result),trace,PageJson(page,end-begin,cursor,false));
        }

        const auto begin = std::min( page.offset, quality.size() );
        const auto end = std::min( begin + page.limit, quality.size() );
        json values = json::array();
        for( size_t index=begin; index<end; index++ ) { const auto& value=quality[index]; values.push_back( {
            { "ref", value.ref }, { "time_ns", Decimal( value.timeNs ) }, { "related_id", Decimal( value.relatedId ) },
            { "counter", value.counter }, { "reason", value.reason }, { "value", Decimal( value.value ) }, { "flags", value.flags } } ); }
        auto result=base(); result["complete"] = bootstrapComplete && dropped == 0;
        result["dropped"] = Decimal( dropped ); result["untracked"] = Decimal( untracked );
        result["quality"] = std::move( values );
        const auto cursor=NextCursor( page, method, trace, end-begin, end<quality.size() );
        return Success( id, std::move( result ), trace, PageJson( page, end-begin, cursor, false ) );
    }

    if( method == "resource.capability" || method == "object.search" || method == "object.get" ||
        method == "asset.search" || method == "asset.get" || method == "mesh.get" || method == "texture.get" ||
        method == "resource.lifecycle" || method == "resource.relations" || method == "resource.memory_impact" ||
        method == "resource.gpu_usage" || method == "resource.physical.occupants" || method == "resource.io_history" || method == "resource.evidence_path" ||
        method == "resource.validation" )
    {
        ...
        auto base = [&]() -> json { return {
            { "present", present }, { "resource_graph_schema_version", resourceGraphSchema },
            { "complete", false }, { "reason", reason }, { "provenance", "exact-binary" },
            { "bootstrap_complete", bootstrapComplete }, { "connection_generation", connectionGeneration },
            { "pre_capture", preCapture }, { "metadata_revision", metadataRevision },
            { "unresolved_reason", json( nullptr ) }, { "dropped", Decimal( dropped ) },
            { "untracked", Decimal( untracked ) },
            { "truncated", truncated }, { "overflow", overflow }, { "runtime_degraded", runtimeDegraded },
            { "trust", "untrusted_trace_data" }
        }; };
        if( method == "resource.capability" && !present )
        {
            auto result = base();
            result["features"] = {
                { "unity_object_asset_identity", false }, { "specialized_mesh_texture_adapters", false },
                { "gfx_logical_physical_chain", false }, { "gpu_pass_usage", false },
                { "physical_allocation_occupants", false }, { "resource_io_context", false },
                { "bounded_evidence_path", false }, { "bounded_bidirectional_evidence_path", false } };
            return Success( id, std::move( result ), trace );
        }
        if( !present )
        {
            auto result = base();
            if( method.ends_with( ".search" ) ) result[method == "object.search" ? "objects" : "assets"] = json::array();
            else if( method == "resource.physical.occupants" )
            {
                result["physical_allocation"] = nullptr;
                result["matched_count"] = Decimal( uint64_t( 0 ) );
                result["occupants"] = json::array();
            }
            else if( method == "resource.validation" ) result["issues"] = json::array();
            else result["resource"] = nullptr;
            return Success( id, std::move( result ), trace );
        }
        ...
        const auto& bootstrap = resourceIndex->bootstrap;
        const auto& quality = resourceIndex->quality;
        bootstrapComplete = !bootstrap.empty() && bootstrap.back().phase == 4 && bootstrap.back().remainingCount == 0;
        if( !bootstrap.empty() )
        {
            connectionGeneration = bootstrap.back().connectionGeneration;
            truncated = bootstrap.back().phase == 5;
        }
        for( const auto& value : objectEvents ) preCapture = preCapture || ( value.flags & 0x06 ) != 0;
        for( const auto& value : metadata ) metadataRevision = std::max( metadataRevision, value.revision );
        for( const auto& value : quality )
        {
            if( value.counter == 1 ) dropped += value.value;
            else if( value.counter == 2 ) overflow = overflow || value.value != 0;
            else if( value.counter == 3 ) untracked += value.value;
            else runtimeDegraded = runtimeDegraded || value.value != 0;
        }
        truncated = truncated || dropped != 0;
        const bool graphComplete = bootstrapComplete && dropped == 0;
        const int64_t effectiveTimeNs = params.value( "at_ns", std::numeric_limits<int64_t>::max() );
        std::unordered_map<uint64_t,uint32_t> activeGfxEpoch;
        for( const auto& value : gfx ) if( value.timeNs <= effectiveTimeNs )
        {
            if( value.eventKind == 1 ) activeGfxEpoch[value.gfxResourceId] = value.generation;
            else if( const auto found = activeGfxEpoch.find( value.gfxResourceId );
                found != activeGfxEpoch.end() && ( value.generation == 0 || found->second == value.generation ) )
                activeGfxEpoch.erase( found );
        }
        std::vector<const analysis::ResourceRelationDto*> latestRelations;
        latestRelations.reserve( resourceIndex->relationHistory.size() );
        for( const auto& [relationKey,history] : resourceIndex->relationHistory )
        {
            auto found = std::upper_bound( history.begin(), history.end(), effectiveTimeNs,
                []( int64_t time, const auto* value ) { return time < value->timeNs; } );
            if( found == history.begin() ) continue;
            latestRelations.push_back( *--found );
        }
        std::vector<const analysis::ResourceRelationDto*> activeRelations;
        activeRelations.reserve( latestRelations.size() );
        for( const auto* value : latestRelations )
        {
            if( ( value->flags >> 6 ) == 2 ) continue;
            if( value->sourceKind == 4 && ( value->sourceEpoch == 0 || !activeGfxEpoch.contains( value->sourceId ) ||
                activeGfxEpoch.at( value->sourceId ) != value->sourceEpoch ) ) continue;
            if( value->targetKind == 4 && ( value->targetEpoch == 0 || !activeGfxEpoch.contains( value->targetId ) ||
                activeGfxEpoch.at( value->targetId ) != value->targetEpoch ) ) continue;
            activeRelations.push_back( value );
        }
        std::vector<const analysis::ResourceRangeDto*> latestRanges;
        latestRanges.reserve( resourceIndex->rangeHistory.size() );
        for( const auto& [rangeKey,history] : resourceIndex->rangeHistory )
        {
            auto found = std::upper_bound( history.begin(), history.end(), effectiveTimeNs,
                []( int64_t time, const auto* value ) { return time < value->timeNs; } );
            if( found == history.begin() ) continue;
            latestRanges.push_back( *--found );
        }
        std::vector<const analysis::ResourceRangeDto*> activeRanges;
        activeRanges.reserve( latestRanges.size() );
        for( const auto* value : latestRanges ) if( ( value->flags >> 6 ) != 2 ) activeRanges.push_back( value );
        ...
        std::unordered_map<uint64_t,std::vector<uint64_t>> assetsByObject, objectsByAsset;
        std::map<std::pair<uint8_t,uint64_t>,std::vector<const analysis::ResourceRelationDto*>> activeRelationsBySource, activeRelationsByTarget;
        std::map<std::pair<uint8_t,uint64_t>,std::vector<const analysis::ResourceRangeDto*>> activeRangesByTarget;
        std::array<std::unordered_map<uint64_t,std::vector<const analysis::ResourceMetadataDto*>>,14> metadataByEntity;
        for(const auto& value:objectEvents)if(value.timeNs<=effectiveTimeNs)objectEventsById[value.objectId].push_back(&value);
        for(const auto& value:roots)if(value.timeNs<=effectiveTimeNs)rootsByObject[value.objectId].push_back(&value);
        for(const auto& value:gfx)if(value.timeNs<=effectiveTimeNs)gfxByTarget[value.targetId].push_back(&value);
        for(const auto& value:parts)if(value.timeNs<=effectiveTimeNs)partsByOwner[value.ownerId].push_back(&value);
        for(const auto* value:activeRanges)
        {
            rangesByPart[value->partId].push_back(value);
            activeRangesByTarget[{value->targetKind,value->targetId}].push_back(value);
        }
        for(const auto& value:assetUpdates)if(value.timeNs<=effectiveTimeNs)updatesByAsset[value.entityId].push_back(&value);
        for(const auto& value:assets)assetByEntity[value.entityId]=&value;
        for(const auto& value:metadata)if(value.timeNs<=effectiveTimeNs&&value.entityKind<metadataByEntity.size())metadataByEntity[value.entityKind][value.entityId].push_back(&value);
        for(const auto* value:activeRelations)
        {
            activeRelationsBySource[{value->sourceKind,value->sourceId}].push_back(value);
            activeRelationsByTarget[{value->targetKind,value->targetId}].push_back(value);
            if(value->sourceKind==1&&value->targetKind==2)
            {assetsByObject[value->targetId].push_back(value->sourceId);objectsByAsset[value->sourceId].push_back(value->targetId);}
        }

        if( method == "resource.capability" )
        {
            auto result = base();
            result["complete"] = graphComplete;
            result["features"] = {
                { "unity_object_asset_identity", true }, { "specialized_mesh_texture_adapters", true },
                { "gfx_logical_physical_chain", true }, { "gpu_pass_usage", true },
                { "physical_allocation_occupants", true }, { "resource_io_context", true },
                { "bounded_evidence_path", true }, { "bounded_bidirectional_evidence_path", true } };
            return Success( id, std::move( result ), trace );
        }

        auto parseNumber = []( const std::string& value, const std::string& prefix ) -> std::optional<uint64_t> {
            if( value.rfind( prefix, 0 ) != 0 ) return std::nullopt;
            try { return std::stoull( value.substr( prefix.size() ) ); } catch( ... ) { return std::nullopt; }
        };
        auto objectIdFromRef = [&]( const std::string& ref ) -> std::optional<uint64_t> {
            if( auto value = parseNumber( ref, "object:" ) ) return value;
            for( const auto& event : objectEvents ) if( event.ref == ref ) return event.objectId;
            return std::nullopt;
        };
        auto assetIdFromRef = [&]( const std::string& ref ) -> std::optional<uint64_t> {
            if( auto value = parseNumber( ref, "asset:" ) ) return value;
            for( const auto& asset : assets ) if( asset.ref == ref ) return asset.entityId;
            return std::nullopt;
        };
        auto metadataJson = [&]( uint64_t entityId, uint8_t entityKind ) {
            json values = json::array();
            if(entityKind<metadataByEntity.size())if(const auto found=metadataByEntity[entityKind].find(entityId);found!=metadataByEntity[entityKind].end())
                for( const auto* value : found->second ) values.push_back( { { "key", value->key }, { "value_kind", value->valueKind },
                    { "value", Decimal( value->value ) }, { "text", value->text.empty() ? json( nullptr ) : json( value->text ) },
                    { "revision", value->revision }, { "time_ns", Decimal( value->timeNs ) } } );
            return values;
        };
        auto objectJson = [&]( uint64_t objectId ) {
            json result = { { "ref", "object:" + std::to_string( objectId ) }, { "object_id", Decimal( objectId ) },
                { "created", false }, { "destroyed", false }, { "updates", json::array() },
                { "assets", json::array() }, { "native_roots", json::array() }, { "gfx_resources", json::array() },
                { "parts", json::array() }, { "metadata", metadataJson( objectId, 2 ) },
                { "name", nullptr }, { "type_name", nullptr }, { "scene_identity", nullptr },
                { "cpu_native_bytes", Decimal( uint64_t( 0 ) ) }, { "unity_reported_gfx_bytes", Decimal( uint64_t( 0 ) ) } };
            uint64_t cpuNativeBytes = 0, unityGfxBytes = 0;
            if(const auto found=objectEventsById.find(objectId);found!=objectEventsById.end())for( const auto* event : found->second )
            {
                const auto& value=*event;
                if( value.eventKind == 1 )
                {
                    result["created"] = true; result["create_time_ns"] = Decimal( value.timeNs );
                    result["instance_id"] = value.instanceId; result["native_pointer"] = Decimal( value.nativePointer );
                    result["runtime_type_index"] = value.runtimeTypeIndex; result["flags"] = value.flags;
                }
                else if( value.eventKind == 2 )
                {
                    result["updates"].push_back( {
                        { "time_ns", Decimal( value.timeNs ) }, { "revision", value.revision }, { "field_mask", value.fieldMask },
                        { "value", Decimal( value.value ) }, { "text", value.text.empty() ? json( nullptr ) : json( value.text ) },
                        { "flags", value.flags } } );
                    if( ( value.fieldMask & 1 ) != 0 && !value.text.empty() ) result["name"] = value.text;
                    if( ( value.fieldMask & 2 ) != 0 && !value.text.empty() ) result["type_name"] = value.text;
                    if( ( value.fieldMask & 8 ) != 0 ) cpuNativeBytes = value.value;
                    if( ( value.fieldMask & 16 ) != 0 ) result["scene_identity"] = Decimal( value.value );
                }
                else { result["destroyed"] = true; result["destroy_time_ns"] = Decimal( value.timeNs ); }
            }
            std::unordered_set<uint64_t> rootIds;
            if(const auto found=assetsByObject.find(objectId);found!=assetsByObject.end())for(const auto value:found->second)
                result["assets"].push_back( "asset:" + std::to_string( value ) );
            if(const auto found=rootsByObject.find(objectId);found!=rootsByObject.end())for( const auto* rootValue : found->second )
            {
                const auto& value=*rootValue;
                rootIds.emplace( value.rootId );
                result["native_roots"].push_back( { { "root_id", Decimal( value.rootId ) }, { "time_ns", Decimal( value.timeNs ) } } );
                if(const auto meta=metadataByEntity[3].find(value.rootId);meta!=metadataByEntity[3].end())for(const auto* item:meta->second)if(item->key==3)
                    cpuNativeBytes = std::max( cpuNativeBytes, item->value );
            }
            auto addGfx=[&](uint64_t target){if(const auto found=gfxByTarget.find(target);found!=gfxByTarget.end())for(const auto* item:found->second)
                result["gfx_resources"].push_back( { { "gfx_resource_id", Decimal( item->gfxResourceId ) },
                    { "target_id", Decimal( item->targetId ) }, { "target_kind", item->targetKind },
                    { "resource_kind", item->resourceKind }, { "event_kind", item->eventKind },
                    { "gfx_lifetime_epoch", item->generation }, { "time_ns", Decimal( item->timeNs ) } } );};
            addGfx(objectId);for(const auto rootId:rootIds)addGfx(rootId);
            if(const auto found=partsByOwner.find(objectId);found!=partsByOwner.end())for( const auto* partValue : found->second ) if(partValue->ownerKind==2)
            {
                const auto& value=*partValue;
                json part = { { "part_id", Decimal( value.partId ) }, { "index", value.index },
                    { "semantic_kind", value.semanticKind }, { "ranges", json::array() } };
                if(const auto rangeFound=rangesByPart.find(value.partId);rangeFound!=rangesByPart.end())for( const auto* range : rangeFound->second ) part["ranges"].push_back( {
                    { "target_id", Decimal( range->targetId ) }, { "target_kind", range->targetKind },
                    { "byte_offset", Decimal( uint64_t( range->byteOffset ) ) },
                    { "byte_length", Decimal( uint64_t( range->byteLength ) ) },
                    { "target_epoch", range->generation }, { "range_revision", range->revision },
                    { "time_ns", Decimal( range->timeNs ) }, { "action", uint8_t( range->flags >> 6 ) } } );
                result["parts"].push_back( std::move( part ) );
            }
            if(const auto meta=metadataByEntity[2].find(objectId);meta!=metadataByEntity[2].end())for( const auto* value : meta->second )
            {
                if( value->key == 3 ) cpuNativeBytes = std::max( cpuNativeBytes, value->value );
                else if( value->key == 4 ) unityGfxBytes = std::max( unityGfxBytes, value->value );
            }
            result["cpu_native_bytes"] = Decimal( cpuNativeBytes );
            result["unity_reported_gfx_bytes"] = Decimal( unityGfxBytes );
            result["live"] = result["created"].get<bool>() && !result["destroyed"].get<bool>();
            result["pre_capture"] = false;
            if(const auto found=objectEventsById.find(objectId);found!=objectEventsById.end())for( const auto* value : found->second ) if( ( value->flags & 0x06 ) != 0 )
            { result["pre_capture"] = true; break; }
            return result;
        };
        auto assetJson = [&]( uint64_t entityId ) {
            json result = { { "ref", "asset:" + std::to_string( entityId ) }, { "entity_id", Decimal( entityId ) },
                { "updates", json::array() }, { "objects", json::array() }, { "metadata", metadataJson( entityId, 1 ) } };
            if(const auto found=assetByEntity.find(entityId);found!=assetByEntity.end())
            {
                const auto& value=*found->second;
                result["asset_id_high"] = Decimal( value.idHigh ); result["asset_id_low"] = Decimal( value.idLow );
                result["identity_namespace"] = value.identityNamespace; result["asset_kind"] = value.assetKind; result["flags"] = value.flags;
            }
            if(const auto found=updatesByAsset.find(entityId);found!=updatesByAsset.end())for( const auto* value : found->second ) result["updates"].push_back( {
                { "field", value->field }, { "value", Decimal( value->value ) },
                { "text", value->text.empty() ? json( nullptr ) : json( value->text ) },
                { "revision", value->revision }, { "time_ns", Decimal( value->timeNs ) } } );
            if(const auto found=objectsByAsset.find(entityId);found!=objectsByAsset.end())for(const auto value:found->second)
                result["objects"].push_back( "object:" + std::to_string( value ) );
            return result;
        };

        if( method == "object.search" )
        {
            auto containsInsensitive = []( std::string value, std::string needle ) {
                std::transform( value.begin(), value.end(), value.begin(), []( unsigned char c ){ return char( std::tolower( c ) ); } );
                std::transform( needle.begin(), needle.end(), needle.begin(), []( unsigned char c ){ return char( std::tolower( c ) ); } );
                return value.find( needle ) != std::string::npos;
            };
            const auto state = params.value( "state", std::string( "all" ) );
            const auto nameFilter = params.value( "name_contains", std::string() );
            const auto typeFilter = params.value( "type_contains", std::string() );
            const auto pathFilter = params.value( "path_contains", std::string() );
            const auto assetFilter = params.value( "asset_ref", std::string() );
            const auto ownerFilter = params.value( "owner_ref", std::string() );
            const auto minCpu = params.value( "min_cpu_bytes", uint64_t( 0 ) );
            const auto minGpu = params.value( "min_gpu_bytes", uint64_t( 0 ) );
            const auto startNs = params.value( "start_ns", std::numeric_limits<int64_t>::min() );
            const auto endNs = params.value( "end_ns", std::numeric_limits<int64_t>::max() );
            std::optional<bool> preCaptureFilter;
            if( params.contains( "pre_capture" ) && params["pre_capture"].is_boolean() ) preCaptureFilter = params["pre_capture"].get<bool>();
            const bool hasFilters = state != "all" || !nameFilter.empty() || !typeFilter.empty() || !pathFilter.empty() ||
                !assetFilter.empty() || !ownerFilter.empty() || minCpu != 0 || minGpu != 0 || preCaptureFilter.has_value() ||
                startNs != std::numeric_limits<int64_t>::min() || endNs != std::numeric_limits<int64_t>::max();
            if( !hasFilters )
            {
                std::vector<uint64_t> order; std::unordered_set<uint64_t> ordered;
                for(const auto& value:objectEvents)if(value.timeNs<=effectiveTimeNs&&ordered.emplace(value.objectId).second)order.emplace_back(value.objectId);
                const auto page=ParsePage(params,method,trace); const auto begin=std::min(page.offset,order.size()), end=std::min(begin+page.limit,order.size());
                json values=json::array();for(size_t index=begin;index<end;index++)values.emplace_back(objectJson(order[index]));
                auto result=base();result["complete"]=graphComplete;result["matched_count"]=Decimal(order.size());result["objects"]=std::move(values);
                const auto cursor=NextCursor(page,method,trace,end-begin,end<order.size());return Success(id,std::move(result),trace,PageJson(page,end-begin,cursor,false));
            }
            std::vector<std::pair<uint64_t,json>> filtered; std::unordered_set<uint64_t> seen;
            for( const auto& value : objectEvents ) if( value.timeNs<=effectiveTimeNs&&seen.emplace( value.objectId ).second )
            {
                auto item = objectJson( value.objectId );
                if( state == "live" && !item["live"].get<bool>() ) continue;
                if( state == "destroyed" && !item["destroyed"].get<bool>() ) continue;
                if( preCaptureFilter && item["pre_capture"].get<bool>() != *preCaptureFilter ) continue;
                if( !nameFilter.empty() && ( item["name"].is_null() || !containsInsensitive( item["name"].get<std::string>(), nameFilter ) ) ) continue;
                if( !typeFilter.empty() && ( item["type_name"].is_null() || !containsInsensitive( item["type_name"].get<std::string>(), typeFilter ) ) ) continue;
                if( std::stoull( item["cpu_native_bytes"].get<std::string>() ) < minCpu ||
                    std::stoull( item["unity_reported_gfx_bytes"].get<std::string>() ) < minGpu ) continue;
                if( !assetFilter.empty() && std::find( item["assets"].begin(), item["assets"].end(), assetFilter ) == item["assets"].end() ) continue;
                if( !ownerFilter.empty() )
                {
                    bool ownerMatch = ownerFilter == item["ref"].get<std::string>();
                    for( const auto& root : item["native_roots"] ) ownerMatch = ownerMatch || ownerFilter == "native-root:" + root["root_id"].get<std::string>();
                    if( !ownerMatch ) continue;
                }
                if( !pathFilter.empty() )
                {
                    bool pathMatch = false;
                    for( const auto& assetRef : item["assets"] ) if( const auto idValue = assetIdFromRef( assetRef.get<std::string>() ) )
                        for( const auto& update : assetUpdates ) if( update.timeNs<=effectiveTimeNs&&update.entityId == *idValue && update.field == 3 && containsInsensitive( update.text, pathFilter ) ) pathMatch = true;
                    if( !pathMatch ) continue;
                }
                int64_t createdNs = std::numeric_limits<int64_t>::min(), destroyedNs = std::numeric_limits<int64_t>::max();
                if( item.contains( "create_time_ns" ) ) createdNs = std::stoll( item["create_time_ns"].get<std::string>() );
                if( item.contains( "destroy_time_ns" ) ) destroyedNs = std::stoll( item["destroy_time_ns"].get<std::string>() );
                if( createdNs > endNs || destroyedNs < startNs ) continue;
                filtered.emplace_back( value.objectId, std::move( item ) );
            }
            const auto page = ParsePage( params, method, trace );
            const auto begin = std::min( page.offset, filtered.size() ), end = std::min( begin + page.limit, filtered.size() );
            json values = json::array(); for( size_t index=begin; index<end; index++ ) values.emplace_back( std::move( filtered[index].second ) );
            auto result=base(); result["complete"]=graphComplete; result["matched_count"]=Decimal(filtered.size()); result["objects"]=std::move(values);
            const auto cursor=NextCursor(page,method,trace,end-begin,end<filtered.size()); return Success(id,std::move(result),trace,PageJson(page,end-begin,cursor,false));
        }
        if( method == "asset.search" )
        {
            const auto page=ParsePage(params,method,trace); const auto begin=std::min(page.offset,assets.size()), end=std::min(begin+page.limit,assets.size());
            json values=json::array(); for(size_t index=begin;index<end;index++) values.emplace_back(assetJson(assets[index].entityId));
            auto result=base(); result["complete"]=graphComplete; result["matched_count"]=Decimal(assets.size()); result["assets"]=std::move(values);
            const auto cursor=NextCursor(page,method,trace,end-begin,end<assets.size()); return Success(id,std::move(result),trace,PageJson(page,end-begin,cursor,false));
        }

        if( method == "resource.validation" )
        {
            std::unordered_map<uint8_t, std::unordered_set<uint64_t>> ids;
            for( const auto& a : assets ) ids[1].emplace(a.entityId); for( const auto& o : objectEvents ) if(o.timeNs<=effectiveTimeNs)ids[2].emplace(o.objectId);
            for( const auto& r : roots ) if(r.timeNs<=effectiveTimeNs)ids[3].emplace(r.rootId); for( const auto& g : gfx ) if(g.timeNs<=effectiveTimeNs)ids[4].emplace(g.gfxResourceId);
            for( const auto& p : parts ) if(p.timeNs<=effectiveTimeNs)ids[7].emplace(p.partId); for( const auto& c : contexts ) if(c.timeNs<=effectiveTimeNs)ids[8].emplace(c.contextId);
            std::unordered_map<uint64_t,bool> objectActive;
            for( const auto& value : objectEvents ) if( value.timeNs <= effectiveTimeNs ) objectActive[value.objectId] = value.eventKind != 3;
            uint64_t unresolvedEdges=0, unresolvedRanges=0, unannotatedRanges=0, wrongEpochRanges=0,
                missingEpochRelations=0, wrongEpochRelations=0, orphanRoots=0, adapterErrors=0;
            std::map<std::string,uint64_t> unresolvedByKind;
            json unresolvedPreview=json::array(), unresolvedRangePreview=json::array(), orphanRootPreview=json::array();
            json missingEpochRelationPreview=json::array(), wrongEpochRelationPreview=json::array(), adapterErrorPreview=json::array();
            auto relationEndEvidence = [&]( const analysis::ResourceRelationDto& value ) {
                const auto found = std::find_if( relations.rbegin(), relations.rend(), [&]( const auto& candidate ) {
                    return ( candidate.flags >> 6 ) == 2 && candidate.sourceKind == value.sourceKind &&
                        candidate.sourceId == value.sourceId && candidate.targetKind == value.targetKind &&
                        candidate.targetId == value.targetId && candidate.relation == value.relation;
                } );
                if( found == relations.rend() ) return json( nullptr );
                return json { {"ref",found->ref},{"source_epoch",found->sourceEpoch},{"target_epoch",found->targetEpoch},
                    {"relation_revision",found->revision},{"time_ns",Decimal(found->timeNs)} };
            };
            for( const auto* relationValue : activeRelations )
            {
                const auto& r = *relationValue;
                const bool sourceExternal = r.sourceKind == 5 || r.sourceKind == 6 || r.sourceKind >= 9;
                const bool targetExternal = r.targetKind == 5 || r.targetKind == 6 || r.targetKind >= 9;
                const bool missingSource=!sourceExternal&&!ids[r.sourceKind].contains(r.sourceId);
                const bool missingTarget=!targetExternal&&!ids[r.targetKind].contains(r.targetId);
                if( missingSource||missingTarget )
                {
                    unresolvedEdges++;
                    const auto signature=std::to_string(r.sourceKind)+"->"+std::to_string(r.targetKind)+"/relation="+std::to_string(r.relation)+
                        (missingSource&&missingTarget?"/both":missingSource?"/source":"/target");
                    unresolvedByKind[signature]++;
                    if( unresolvedPreview.size() < 32 )
                    {
                        json preview={{"ref",r.ref},{"source_kind",r.sourceKind},{"source_id",Decimal(r.sourceId)},
                            {"target_kind",r.targetKind},{"target_id",Decimal(r.targetId)},{"relation",r.relation},
                            {"relation_revision",r.revision},{"relation_time_ns",Decimal(r.timeNs)},
                            {"missing_source",missingSource},{"missing_target",missingTarget},
                            {"provenance",uint8_t(r.provenance&0x0F)},{"flags",uint8_t(r.flags&0x3F)},
                            {"matching_end",relationEndEvidence(r)}};
                        if( r.sourceKind == 8 )
                        {
                            const auto context = std::find_if( contexts.rbegin(), contexts.rend(), [&]( const auto& value ) {
                                return value.contextId == r.sourceId;
                            } );
                            preview["last_source_context"] = context == contexts.rend() ? json(nullptr) : json {
                                {"stage",context->stage},{"time_ns",Decimal(context->timeNs)},
                                {"related_kind",context->relatedKind},{"related_id",Decimal(context->relatedId)} };
                        }
                        unresolvedPreview.push_back(std::move(preview));
                    }
                }
            }
            for( const auto* relationPointer : latestRelations ) if( ( relationPointer->flags >> 6 ) != 2 )
            {
                const auto& relationValue = *relationPointer;
                if( relationValue.sourceKind == 4 )
                {
                    if( relationValue.sourceEpoch == 0 )
                    {
                        missingEpochRelations++;
                        if( missingEpochRelationPreview.size() < 32 ) missingEpochRelationPreview.push_back( {
                            {"ref",relationValue.ref},{"relation",relationValue.relation},{"endpoint","source"},
                            {"gfx_resource_id",Decimal(relationValue.sourceId)},{"relation_epoch",relationValue.sourceEpoch} } );
                    }
                    else if( const auto found=activeGfxEpoch.find( relationValue.sourceId );
                        found==activeGfxEpoch.end() || found->second!=relationValue.sourceEpoch )
                    {
                        wrongEpochRelations++;
                        if( wrongEpochRelationPreview.size() < 32 ) wrongEpochRelationPreview.push_back( {
                            {"ref",relationValue.ref},{"relation",relationValue.relation},{"endpoint","source"},
                            {"source_kind",relationValue.sourceKind},{"source_id",Decimal(relationValue.sourceId)},
                            {"target_kind",relationValue.targetKind},{"target_id",Decimal(relationValue.targetId)},
                            {"gfx_resource_id",Decimal(relationValue.sourceId)},{"relation_epoch",relationValue.sourceEpoch},
                            {"active_epoch",found==activeGfxEpoch.end()?json(nullptr):json(found->second)},
                            {"matching_end",relationEndEvidence(relationValue)} } );
                    }
                }
                if( relationValue.targetKind == 4 )
                {
                    if( relationValue.targetEpoch == 0 )
                    {
                        missingEpochRelations++;
                        if( missingEpochRelationPreview.size() < 32 ) missingEpochRelationPreview.push_back( {
                            {"ref",relationValue.ref},{"relation",relationValue.relation},{"endpoint","target"},
                            {"gfx_resource_id",Decimal(relationValue.targetId)},{"relation_epoch",relationValue.targetEpoch} } );
                    }
                    else if( const auto found=activeGfxEpoch.find( relationValue.targetId );
                        found==activeGfxEpoch.end() || found->second!=relationValue.targetEpoch )
                    {
                        wrongEpochRelations++;
                        if( wrongEpochRelationPreview.size() < 32 ) wrongEpochRelationPreview.push_back( {
                            {"ref",relationValue.ref},{"relation",relationValue.relation},{"endpoint","target"},
            ...
            for( const auto* rangeValue : activeRanges ) if( rangeValue->targetKind == 4 )
            {
                const auto& r = *rangeValue;
                if( r.revision == 0 || r.timeNs == 0 )
                { unannotatedRanges++; if(unresolvedRangePreview.size()<32)unresolvedRangePreview.push_back({{"ref",r.ref},{"part_id",Decimal(r.partId)},{"target_id",Decimal(r.targetId)},{"reason","missing_range_revision_timestamp"}}); }
                else if( !ids[4].contains(r.targetId) )
                { unresolvedRanges++; if(unresolvedRangePreview.size()<32)unresolvedRangePreview.push_back({{"ref",r.ref},{"part_id",Decimal(r.partId)},{"target_id",Decimal(r.targetId)},{"target_epoch",r.generation}}); }
                else if( const auto found=activeGfxEpoch.find(r.targetId); found==activeGfxEpoch.end() || found->second!=r.generation )
                { wrongEpochRanges++; if(unresolvedRangePreview.size()<32)unresolvedRangePreview.push_back({{"ref",r.ref},{"part_id",Decimal(r.partId)},{"target_id",Decimal(r.targetId)},
                    {"target_epoch",r.generation},{"active_epoch",found==activeGfxEpoch.end()?json(nullptr):json(found->second)}}); }
            }
            for( const auto* relationValue : activeRelations ) if( relationValue->relation == 2 && relationValue->sourceKind == 2 && relationValue->targetKind == 3 &&
                ( !objectActive.contains(relationValue->sourceId) || !objectActive[relationValue->sourceId] ) )
            {
                orphanRoots++;
                if( orphanRootPreview.size() < 32 )
                {
                    json lastObjectEvent=nullptr;
                    if( const auto found=objectEventsById.find(relationValue->sourceId); found!=objectEventsById.end() && !found->second.empty() )
                    {
                        const auto* event=*std::max_element(found->second.begin(),found->second.end(),[](const auto* lhs,const auto* rhs){return lhs->timeNs<rhs->timeNs;});
                        lastObjectEvent={{"event_kind",event->eventKind},{"time_ns",Decimal(event->timeNs)},{"revision",event->revision}};
                    }
                    orphanRootPreview.push_back({{"ref",relationValue->ref},{"root_id",Decimal(relationValue->targetId)},
                        {"object_id",Decimal(relationValue->sourceId)},{"relation_revision",relationValue->revision},
                        {"relation_time_ns",Decimal(relationValue->timeNs)},{"matching_end",relationEndEvidence(*relationValue)},
                        {"last_object_event",std::move(lastObjectEvent)}});
                }
            }
            for( const auto& [objectIdValue,events] : objectEventsById )
            {
                const auto object = objectJson(objectIdValue);
                // Adapter coverage is a current-state invariant. Historical
                // objects that were already destroyed before capture may have
                // no adapter metadata and are still valid lifecycle evidence.
                if( !object.value("live",false) ) continue;
                if( object["type_name"].is_null() ) continue;
                const auto typeName = object["type_name"].get<std::string>();
                uint64_t adapterKind = 0;
                if( const auto found=metadataByEntity[2].find(objectIdValue); found!=metadataByEntity[2].end() )
                    for( const auto* value : found->second ) if( value->key==72 ) adapterKind=value->value;
                const bool meshExpected=typeName=="Mesh";
                const bool textureExpected=typeName=="Texture2D"||typeName=="Texture3D"||typeName=="Texture2DArray"||
                    typeName=="Cubemap"||typeName=="CubemapArray"||typeName=="SparseTexture";
                const bool renderTextureExpected=typeName=="RenderTexture";
                // RenderTexture-backed project adapters retain their concrete
                // role (VSM/VG/ScreenProbe/TSR/GPUDriven) in AdapterKind. They
                // are texture resources, not mismatches against the generic
                // RenderTexture value.
                const bool renderTextureAdapter=adapterKind>=4 && adapterKind<=9;
                if( (meshExpected&&adapterKind!=1)||(textureExpected&&adapterKind!=2)||(renderTextureExpected&&!renderTextureAdapter) )
                {
                    adapterErrors++;
                    if( adapterErrorPreview.size() < 32 ) adapterErrorPreview.push_back( {
                        {"object_id",Decimal(objectIdValue)},{"ref","object:"+std::to_string(objectIdValue)},
                        {"type_name",typeName},{"adapter_kind",adapterKind},
                        {"expected",meshExpected?"Mesh":textureExpected?"Texture":"RenderTextureOrSpecialized"} } );
                }
            }
            auto result=base(); result["complete"]=graphComplete&&unresolvedEdges==0&&unresolvedRanges==0&&unannotatedRanges==0&&wrongEpochRanges==0&&
                missingEpochRelations==0&&wrongEpochRelations==0&&orphanRoots==0&&adapterErrors==0;
            result["counts"]={{"unresolved_edges",Decimal(unresolvedEdges)},{"unresolved_ranges",Decimal(unresolvedRanges)},
                {"unannotated_ranges",Decimal(unannotatedRanges)},{"wrong_epoch_ranges",Decimal(wrongEpochRanges)},{"orphan_roots",Decimal(orphanRoots)},
                {"missing_epoch_relations",Decimal(missingEpochRelations)},{"wrong_epoch_relations",Decimal(wrongEpochRelations)},
                {"adapter_errors",Decimal(adapterErrors)},{"dropped",Decimal(dropped)},{"untracked",Decimal(untracked)}};
            result["unresolved_by_kind"]=json::object();for(const auto& [signature,count]:unresolvedByKind)result["unresolved_by_kind"][signature]=Decimal(count);
            result["unresolved_preview"]=std::move(unresolvedPreview);result["unresolved_range_preview"]=std::move(unresolvedRangePreview);
            result["missing_epoch_relation_preview"]=std::move(missingEpochRelationPreview);
            result["wrong_epoch_relation_preview"]=std::move(wrongEpochRelationPreview);
            result["orphan_root_preview"]=std::move(orphanRootPreview);
            result["adapter_error_preview"]=std::move(adapterErrorPreview);
            result["issues"]=json::array(); if(unresolvedEdges) result["issues"].push_back("unresolved_relation_endpoint");
            if(unresolvedRanges) result["issues"].push_back("unresolved_part_range_target"); if(unannotatedRanges) result["issues"].push_back("range_missing_revision_timestamp");
            if(wrongEpochRanges) result["issues"].push_back("range_wrong_gfx_epoch");
            if(missingEpochRelations) result["issues"].push_back("relation_missing_gfx_epoch");
            if(wrongEpochRelations) result["issues"].push_back("relation_wrong_gfx_epoch");
            if(orphanRoots) result["issues"].push_back("orphan_native_root"); if(adapterErrors) result["issues"].push_back("adapter_kind_mismatch");
            if(dropped) result["issues"].push_back("telemetry_dropped"); return Success(id,std::move(result),trace);
        }
        ...
        const auto objectId = objectIdFromRef( requested ); const auto assetId = assetIdFromRef( requested );
        auto genericEntity = [&]() -> std::optional<std::pair<uint8_t,uint64_t>> {
            static constexpr std::pair<const char*,uint8_t> prefixes[] = {
                {"native-root:",3},{"gfx-resource:",4},{"logical-resource:",5},{"physical-allocation:",6},
                {"resource-part:",7},{"resource-context:",8},{"gpu-pass:",9},{"gpu-memory-pass:",9},
                {"io-request:",10},{"job:",11},{"frame:",13} };
            for( const auto& [prefix,kind] : prefixes ) if( const auto value=parseNumber(requested,prefix) ) return std::pair<uint8_t,uint64_t>{kind,*value};
            return std::nullopt;
        }();
        if( !objectId && !assetId && !genericEntity ) throw QueryError("ENTITY_NOT_FOUND","resource ref was not found in this trace");
        const uint8_t selectedKind = objectId ? 2 : assetId ? 1 : genericEntity->first;
        const uint64_t selectedId = objectId ? *objectId : assetId ? *assetId : genericEntity->second;
        auto selectedJson = [&]() {
            if( objectId ) return objectJson(*objectId);
            if( assetId ) return assetJson(*assetId);
            return json{{"ref",requested},{"entity_kind",selectedKind},{"entity_id",Decimal(selectedId)},
                {"metadata",metadataJson(selectedId,selectedKind)}};
        };

        if( method == "object.get" || method == "mesh.get" || method == "texture.get" || method == "asset.get" )
        {
            if( method == "asset.get" && !assetId ) throw QueryError("INVALID_PARAMS","asset.get requires an asset ref");
            if( method != "asset.get" && !objectId ) throw QueryError("INVALID_PARAMS","object/mesh/texture get requires an object ref");
            bool meshObject = false, textureObject = false;
            if( objectId )
            {
                for( const auto& value : metadata ) if( value.entityKind == 2 && value.entityId == *objectId && value.key == 72 )
                {
                    meshObject = value.value == 1;
                    textureObject = value.value == 2 || ( value.value >= 4 && value.value <= 9 );
                }
            }
            if( method == "mesh.get" && !meshObject ) throw QueryError("ENTITY_KIND_MISMATCH","mesh.get requires a Mesh resource object");
            if( method == "texture.get" && !textureObject ) throw QueryError("ENTITY_KIND_MISMATCH","texture.get requires a Texture or RenderTexture resource object");
            auto resource = selectedJson();
            if( method == "object.get" && objectId )
            {
                std::set<std::pair<uint8_t,uint64_t>> reachable;
                std::vector<std::pair<uint8_t,uint64_t>> pending={{2,*objectId}}; reachable.emplace(2,*objectId);
                for(size_t cursor=0;cursor<pending.size()&&pending.size()<4096;cursor++)
                {
                    const auto outgoing=activeRelationsBySource.find(pending[cursor]);
                    if(outgoing==activeRelationsBySource.end())continue;
                    for(const auto* edge:outgoing->second)
                    {
                        const std::pair<uint8_t,uint64_t> next { edge->targetKind, edge->targetId };
                        if(reachable.emplace(next).second)pending.emplace_back(next);
                    }
                }
                auto localReachable=[&](uint8_t kind,uint64_t value){return reachable.contains(std::pair<uint8_t,uint64_t>{kind,value});};
                resource["contexts"]=json::array(); resource["io_request_ids"]=json::array(); resource["gpu_pass_ids"]=json::array();
                for(const auto& value:contexts) if(value.timeNs<=effectiveTimeNs&&localReachable(8,value.contextId))
                {
                    resource["contexts"].push_back({{"context_id",Decimal(value.contextId)},{"stage",value.stage},
                        {"related_kind",value.relatedKind},{"related_id",Decimal(value.relatedId)},{"callsite_id",value.callsiteId},{"time_ns",Decimal(value.timeNs)}});
                    if(value.relatedKind==10&&value.relatedId)resource["io_request_ids"].push_back(Decimal(value.relatedId));
                }
                for(const auto* edge:activeRelations)
                {
                    if(edge->sourceKind==9&&edge->targetKind==5&&localReachable(5,edge->targetId))resource["gpu_pass_ids"].push_back(Decimal(edge->sourceId));
                    if(edge->targetKind==10&&localReachable(edge->sourceKind,edge->sourceId))resource["io_request_ids"].push_back(Decimal(edge->targetId));
                }
                resource["memory_summary"]={{"cpu_native_owned_bytes",resource["cpu_native_bytes"]},
                    {"unity_reported_gfx_bytes",resource["unity_reported_gfx_bytes"]},
                    {"gpu_logical_bytes",nullptr},{"gpu_physical_bytes",nullptr},
                    {"detail_method","resource.memory_impact"}};
                resource["quality"]={{"complete",graphComplete},{"pre_capture",resource["pre_capture"]},
                    {"provenance","exact-engine"},{"metadata_revision",metadataRevision},{"dropped",Decimal(dropped)}};
            }
            auto result=base(); result["complete"]=graphComplete; result["resource"]=std::move(resource);
            result["view"] = method == "mesh.get" ? "mesh" : method == "texture.get" ? "texture" : method == "asset.get" ? "asset" : "object";
            return Success(id,std::move(result),trace);
        }

        std::set<std::pair<uint8_t,uint64_t>> reachableKeys;
        reachableKeys.emplace(selectedKind,selectedId);
        std::vector<std::pair<uint8_t,uint64_t>> frontier={{selectedKind,selectedId}};
        for(size_t cursor=0;cursor<frontier.size() && frontier.size()<4096;cursor++)
        {
            const auto outgoing=activeRelationsBySource.find(frontier[cursor]);
            if(outgoing==activeRelationsBySource.end())continue;
            for(const auto* r:outgoing->second)
            {
                const std::pair<uint8_t,uint64_t> next { r->targetKind, r->targetId };
                if(reachableKeys.emplace(next).second) frontier.emplace_back(next);
            }
        }
        auto isReachable=[&](uint8_t kind,uint64_t value){return reachableKeys.contains(std::pair<uint8_t,uint64_t>{kind,value});};

        if( method == "resource.lifecycle" )
        {
            json events=json::array();
            if(objectId) for(const auto& v:objectEvents) if(v.timeNs<=effectiveTimeNs&&v.objectId==*objectId) events.push_back({{"ref",v.ref},{"event_kind",v.eventKind},{"time_ns",Decimal(v.timeNs)},{"revision",v.revision},{"flags",v.flags}});
            if(assetId) for(const auto& v:assetUpdates) if(v.timeNs<=effectiveTimeNs&&v.entityId==*assetId) events.push_back({{"ref",v.ref},{"event_kind","asset_update"},{"field",v.field},{"time_ns",Decimal(v.timeNs)},{"revision",v.revision},{"flags",v.flags}});
            for(const auto& v:gfx) if(v.timeNs<=effectiveTimeNs&&isReachable(4,v.gfxResourceId)) events.push_back({{"ref",v.ref},{"event_kind",v.eventKind==1?"gfx_bind":"gfx_unbind"},{"gfx_resource_id",Decimal(v.gfxResourceId)},{"gfx_lifetime_epoch",v.generation},{"time_ns",Decimal(v.timeNs)}});
            auto result=base(); result["complete"]=graphComplete; result["resource"]=selectedJson(); result["events"]=std::move(events); return Success(id,std::move(result),trace);
        }
        if( method == "resource.relations" )
        {
            json values=json::array(); for(const auto* v:activeRelations) if(isReachable(v->sourceKind,v->sourceId)&&isReachable(v->targetKind,v->targetId)) values.push_back({
                {"ref",v->ref},{"source_kind",v->sourceKind},{"source_id",Decimal(v->sourceId)},{"target_kind",v->targetKind},{"target_id",Decimal(v->targetId)},
                {"relation",v->relation},{"relation_revision",v->revision},{"action",uint8_t(v->flags>>6)},
                {"source_epoch",v->sourceEpoch},{"target_epoch",v->targetEpoch},
                {"provenance",uint8_t(v->provenance&0x0F)},{"end_reason",uint8_t(v->provenance>>4)},
                {"time_ns",Decimal(v->timeNs)},{"flags",uint8_t(v->flags&0x3F)}});
            auto result=base(); result["complete"]=graphComplete; result["resource"]=selectedJson(); result["relations"]=std::move(values); return Success(id,std::move(result),trace);
        }

        const auto gpu = CachedGpuAttribution( trace.id, source, false );
        std::unordered_set<uint64_t> logicalIds; for(const auto& v:gpu->logicalResources) if(isReachable(5,v.logicalResourceId)||isReachable(4,v.logicalResourceId)) logicalIds.emplace(v.logicalResourceId);
        for(const auto* v:activeRelations) if(v->targetKind==5&&isReachable(v->sourceKind,v->sourceId)) logicalIds.emplace(v->targetId);
        if( selectedKind == 5 ) logicalIds.emplace( selectedId );
        if( selectedKind == 6 ) for( const auto& value : gpu->logicalResources ) if( value.physicalAllocationId == selectedId ) logicalIds.emplace( value.logicalResourceId );
        if( selectedKind == 9 || selectedKind == 13 ) for( const auto& pass : gpu->passes )
            if( ( selectedKind == 9 && pass.passId == selectedId ) || ( selectedKind == 13 && pass.frame == selectedId ) )
                for( const auto& use : pass.uses ) logicalIds.emplace( use.allocationId );
        if( method == "resource.physical.occupants" )
        {
            if( selectedKind != 6 ) throw QueryError( "INVALID_PARAMS", "resource.physical.occupants requires a physical-allocation ref" );
            std::vector<const analysis::GpuMemoryLogicalResource*> occupants;
            for( const auto& value : gpu->logicalResources ) if( value.physicalAllocationId == selectedId ) occupants.push_back( &value );
            std::sort( occupants.begin(), occupants.end(), []( const auto* lhs, const auto* rhs ) {
                return lhs->logicalResourceId < rhs->logicalResourceId;
            } );

            const auto page = ParsePage( params, method, trace );
            const auto begin = std::min( page.offset, occupants.size() );
            const auto end = std::min( begin + page.limit, occupants.size() );
            json values = json::array();
            for( size_t index=begin; index<end; index++ )
            {
                const auto& logical = *occupants[index];
                std::set<std::pair<uint8_t,uint64_t>> ancestors;
                std::vector<std::pair<uint8_t,uint64_t>> pending={{5,logical.logicalResourceId}};
                ancestors.emplace(5,logical.logicalResourceId);
                bool originTruncated=false;
                for( size_t cursor=0; cursor<pending.size(); cursor++ )
                {
                    if( pending.size() >= 4096 ) { originTruncated=true; break; }
                    const auto current=pending[cursor];
                    if( const auto incoming=activeRelationsByTarget.find(current); incoming!=activeRelationsByTarget.end() )
                        for( const auto* edge : incoming->second )
                        {
                            const std::pair<uint8_t,uint64_t> sourceKey { edge->sourceKind, edge->sourceId };
                            if( ancestors.emplace(sourceKey).second ) pending.emplace_back(sourceKey);
                        }
                    if( current.first == 4 )
                    {
                        if( const auto ranges=activeRangesByTarget.find(current); ranges!=activeRangesByTarget.end() )
                            for( const auto* range : ranges->second )
                            {
                                const std::pair<uint8_t,uint64_t> partKey { 7, range->partId };
                                if( ancestors.emplace(partKey).second ) pending.emplace_back(partKey);
                            }
                    }
                    if( current.first == 5 ) for( const auto& binding : gfx )
                    {
                        if( binding.timeNs > effectiveTimeNs || binding.eventKind != 1 || binding.targetKind != 5 ||
                            binding.targetId != current.second ) continue;
                        const auto active=activeGfxEpoch.find(binding.gfxResourceId);
                        if( active==activeGfxEpoch.end() || active->second!=binding.generation ) continue;
                        const std::pair<uint8_t,uint64_t> gfxKey { 4, binding.gfxResourceId };
                        if( ancestors.emplace(gfxKey).second ) pending.emplace_back(gfxKey);
                    }
                }
                json assetRefs=json::array(), objectRefs=json::array(), rootRefs=json::array(), gfxRefs=json::array(), partRefs=json::array();
                for( const auto& [kind,value] : ancestors )
                {
                    if( kind==1 ) assetRefs.push_back("asset:"+std::to_string(value));
                    else if( kind==2 ) objectRefs.push_back("object:"+std::to_string(value));
                    else if( kind==3 ) rootRefs.push_back("native-root:"+std::to_string(value));
                    else if( kind==4 ) gfxRefs.push_back("gfx-resource:"+std::to_string(value));
                    else if( kind==7 ) partRefs.push_back("resource-part:"+std::to_string(value));
                }
                values.push_back({
                    {"ref","logical-resource:"+std::to_string(logical.logicalResourceId)},
                    {"logical_resource_id",Decimal(logical.logicalResourceId)},
                    {"physical_allocation_id",Decimal(logical.physicalAllocationId)},
                    {"physical_offset",Decimal(logical.physicalOffset)}, {"size_bytes",Decimal(logical.size)},
                    {"name",logical.name}, {"kind",std::string(1,logical.kind)}, {"segment",std::string(1,logical.segment)},
                    {"primary_owner_id",logical.primaryOwnerId}, {"physical_owner_id",logical.physicalOwnerId},
                    {"asset_refs",std::move(assetRefs)}, {"object_refs",std::move(objectRefs)},
                    {"native_root_refs",std::move(rootRefs)}, {"gfx_resource_refs",std::move(gfxRefs)},
                    {"part_refs",std::move(partRefs)}, {"origin_complete",!originTruncated},
                    {"provenance","exact-resource-graph-reverse"}
                });
            }
            values = ProjectFields( std::move(values), params );
            json allocation={{"ref",requested},{"allocation_id",Decimal(selectedId)}};
            if( const auto found=std::find_if(gpu->allocations.begin(),gpu->allocations.end(),[&](const auto& value){return value.allocation.allocationId==selectedId;}); found!=gpu->allocations.end() )
            {
                allocation["size_bytes"]=Decimal(found->allocation.size);
                allocation["pool_name"]=found->allocation.poolName;
                allocation["allocation_ns"]=Decimal(found->allocation.allocationNs);
                allocation["free_ns"]=found->allocation.freeNs?json(Decimal(*found->allocation.freeNs)):json(nullptr);
                allocation["allocation_callstack"]=found->allocation.allocationCallstack;
            }
            auto result=base(); result["complete"]=graphComplete&&gpu->complete; result["physical_allocation"]=std::move(allocation);
            result["matched_count"]=Decimal(occupants.size()); result["occupants"]=std::move(values);
            const auto cursor=NextCursor(page,method,trace,end-begin,end<occupants.size());
            return Success(id,std::move(result),trace,PageJson(page,end-begin,cursor,false));
        }
        if( method == "resource.memory_impact" )
        {
            uint64_t logicalBytes=0, physicalExclusiveBytes=0, physicalSharedContainerBytes=0,
                physicalCoveredRangeBytes=0, residentBytes=0, workingSetBytes=0;
            std::unordered_set<uint64_t> physicalIds, workingSetIds;
            std::unordered_map<uint64_t,uint64_t> physicalLogicalTotal, physicalLogicalSelected;
            std::unordered_map<uint64_t,std::vector<std::pair<uint64_t,uint64_t>>> selectedPhysicalRanges;
            for( const auto& value : gpu->logicalResources ) if( value.physicalAllocationId ) physicalLogicalTotal[value.physicalAllocationId]++;
            json logical=json::array();
            for(const auto& v:gpu->logicalResources) if(logicalIds.contains(v.logicalResourceId))
            {
                logicalBytes+=v.size; if(v.physicalAllocationId){
                    physicalIds.emplace(v.physicalAllocationId); physicalLogicalSelected[v.physicalAllocationId]++;
                    const uint64_t rangeEnd = v.physicalOffset > std::numeric_limits<uint64_t>::max() - v.size ?
                        std::numeric_limits<uint64_t>::max() : v.physicalOffset + v.size;
                    selectedPhysicalRanges[v.physicalAllocationId].emplace_back(v.physicalOffset,rangeEnd);
                }
                logical.push_back({{"logical_resource_id",Decimal(v.logicalResourceId)},{"physical_allocation_id",Decimal(v.physicalAllocationId)},
                    {"size_bytes",Decimal(v.size)},{"physical_offset",Decimal(v.physicalOffset)},{"name",v.name},{"kind",std::string(1,v.kind)}});
            }
            for(const auto& v:gpu->allocations) if(physicalIds.contains(v.allocation.allocationId))
            {
                if( physicalLogicalSelected[v.allocation.allocationId] == physicalLogicalTotal[v.allocation.allocationId] ) physicalExclusiveBytes += v.allocation.size;
                else physicalSharedContainerBytes += v.allocation.size;
            }
            for( auto& [allocationId,intervals] : selectedPhysicalRanges )
            {
                std::sort( intervals.begin(), intervals.end() );
                uint64_t begin=0, end=0; bool open=false;
                for( const auto& interval : intervals )
                {
                    if( !open ) { begin=interval.first; end=interval.second; open=true; continue; }
                    if( interval.first <= end ) end=std::max(end,interval.second);
                    else { physicalCoveredRangeBytes += end-begin; begin=interval.first; end=interval.second; }
                }
                if( open ) physicalCoveredRangeBytes += end-begin;
            }
            std::unordered_map<uint64_t,const analysis::GpuMemoryResidencyEvent*> latestResidency;
            for( const auto& value : gpu->residencyEvents )
            {
                const auto found = latestResidency.find( value.allocationId );
                if( found == latestResidency.end() || found->second->timeNs < value.timeNs ) latestResidency[value.allocationId] = &value;
            }
            for( const auto& [allocationId,event] : latestResidency ) if( physicalIds.contains( allocationId ) && event->state == 'R' ) residentBytes += event->size;
            for( const auto& pass : gpu->passes ) for( const auto& use : pass.uses )
                if( logicalIds.contains( use.allocationId ) && use.resolvedPhysicalAllocationId ) workingSetIds.emplace( use.resolvedPhysicalAllocationId );
            for( const auto& value : gpu->allocations ) if( workingSetIds.contains( value.allocation.allocationId ) ) workingSetBytes += value.allocation.size;
            auto resource = selectedJson();
            const json cpuNative = resource.contains("cpu_native_bytes") ? resource["cpu_native_bytes"] : json(nullptr);
            const json unityGfx = resource.contains("unity_reported_gfx_bytes") ? resource["unity_reported_gfx_bytes"] : json(nullptr);
            auto result=base(); result["complete"]=graphComplete&&gpu->complete; result["resource"]=std::move(resource);
            result["memory"]={{"cpu_native_owned_bytes",cpuNative},{"cpu_allocation_bytes",nullptr},{"unity_reported_gfx_bytes",unityGfx},
                {"gpu_logical_bytes",Decimal(logicalBytes)},{"gpu_physical_exclusive_bytes",Decimal(physicalExclusiveBytes)},
                {"gpu_physical_shared_container_bytes",Decimal(physicalSharedContainerBytes)},
                {"gpu_physical_covered_range_bytes",Decimal(physicalCoveredRangeBytes)},{"gpu_resident_bytes",Decimal(residentBytes)},
                {"pass_referenced_working_set_bytes",Decimal(workingSetBytes)},
                {"direct",{{"gpu_logical_bytes",Decimal(logicalBytes)},{"physical_exclusive_bytes",Decimal(physicalExclusiveBytes)},
                    {"physical_shared_container_bytes",Decimal(physicalSharedContainerBytes)},
                    {"physical_covered_range_bytes",Decimal(physicalCoveredRangeBytes)}}},
                {"inclusive",{{"gpu_logical_bytes",Decimal(logicalBytes)},{"physical_exclusive_bytes",Decimal(physicalExclusiveBytes)},
                    {"physical_shared_container_bytes",Decimal(physicalSharedContainerBytes)},
                    {"physical_covered_range_bytes",Decimal(physicalCoveredRangeBytes)}}},
                {"unknown_unclassified_bytes",nullptr},{"aggregation_rule","dimensions_are_not_summed"}};
            result["logical_resources"]=std::move(logical);
            result["unavailable"]={"cpu_allocation_requires_allocator_to_object_relation","unknown_unclassified_requires_object_scoped_dxgi_reconciliation"};
            return Success(id,std::move(result),trace);
        }
        if( method == "resource.gpu_usage" )
        {
            std::vector<const analysis::GpuMemoryPass*> matchingPasses;
            std::unordered_set<uint64_t> physicalIds; int64_t firstUse=std::numeric_limits<int64_t>::max(), lastUse=std::numeric_limits<int64_t>::min();
            for(const auto& v:gpu->logicalResources) if(logicalIds.contains(v.logicalResourceId)&&v.physicalAllocationId) physicalIds.emplace(v.physicalAllocationId);
            for(const auto& pass:gpu->passes)
            {
                const bool matches=std::any_of(pass.uses.begin(),pass.uses.end(),[&](const auto& use){
                    return logicalIds.contains(use.allocationId)||physicalIds.contains(use.resolvedPhysicalAllocationId); });
                if(matches)
                {
                    firstUse=std::min(firstUse,pass.start); lastUse=std::max(lastUse,pass.end);
                    matchingPasses.push_back(&pass);
                }
            }
            const auto page=ParsePage(params,method,trace);
            const auto begin=std::min(page.offset,matchingPasses.size()), end=std::min(begin+page.limit,matchingPasses.size());
            json passes=json::array();
            for(size_t index=begin;index<end;index++)
            {
                    const auto& pass=*matchingPasses[index];
                    json uses=json::array(); for(const auto& use:pass.uses) if(logicalIds.contains(use.allocationId)||physicalIds.contains(use.resolvedPhysicalAllocationId)) uses.push_back({
                        {"logical_resource_id",Decimal(use.allocationId)},{"physical_allocation_id",Decimal(use.resolvedPhysicalAllocationId)},
                        {"usage_mask",use.usageMask},{"resource_set_id",use.resourceSetId},{"encoding",use.encoding}});
                    json zoneRef=nullptr, contextRef=nullptr, queueType=nullptr, queueName=nullptr;
                    if( pass.gpuZoneIndex ) if( const auto ref = source->GetGpuZoneRef( *pass.gpuZoneIndex ) )
                    {
                        zoneRef=*ref;
                        if( const auto zone=source->GetGpuZone(*ref) )
                        {
                            contextRef=zone->contextRef;
                            for(const auto& context:source->GetGpuContexts()) if(context.ref==zone->contextRef)
                            { queueType=context.typeName; queueName=context.name; break; }
                        }
                    }
                    passes.push_back({{"ref","gpu-memory-pass:"+std::to_string(pass.passId)},{"pass_ref","gpu-memory-pass:"+std::to_string(pass.passId)},{"pass_id",Decimal(pass.passId)},
                        {"frame",Decimal(pass.frame)},{"taxonomy_id",pass.labelId},{"name",pass.name},{"start_ns",Decimal(pass.start)},{"end_ns",Decimal(pass.end)},
                        {"gpu_zone_ref",std::move(zoneRef)},{"queue_context_ref",std::move(contextRef)},
                        {"queue_type",std::move(queueType)},{"queue_name",std::move(queueName)},
                        {"provenance",pass.gpuPairing==analysis::GpuZonePairing::Exact?"exact-d3d12":"derived-exact-rollup"},
                        {"complete",pass.complete},{"truncated",pass.truncated},{"uses",std::move(uses)}});
            }
            passes=ProjectFields(std::move(passes),params);
            json resourceRanges=json::array(); for(const auto& part:parts) if(isReachable(7,part.partId)) for(const auto* range:activeRanges) if(range->partId==part.partId)
                resourceRanges.push_back({{"part_id",Decimal(range->partId)},{"target_id",Decimal(range->targetId)},
                    {"target_kind",range->targetKind},{"byte_offset",Decimal(uint64_t(range->byteOffset))},{"byte_length",Decimal(uint64_t(range->byteLength))},
                    {"target_epoch",range->generation},{"range_revision",range->revision},{"time_ns",Decimal(range->timeNs)},
                    {"action",uint8_t(range->flags>>6)}});
            auto result=base(); result["complete"]=graphComplete&&gpu->complete; result["resource"]=selectedJson(); result["logical_resource_count"]=Decimal(logicalIds.size());
            result["first_use_ns"]=firstUse==std::numeric_limits<int64_t>::max()?json(nullptr):json(Decimal(firstUse));
            result["last_use_ns"]=lastUse==std::numeric_limits<int64_t>::min()?json(nullptr):json(Decimal(lastUse));
            result["physical_allocation_ids"]=json::array(); for(auto value:physicalIds) result["physical_allocation_ids"].push_back(Decimal(value));
            result["resource_ranges"]=std::move(resourceRanges); result["matched_count"]=Decimal(matchingPasses.size()); result["passes"]=std::move(passes);
            const auto cursor=NextCursor(page,method,trace,end-begin,end<matchingPasses.size());
            return Success(id,std::move(result),trace,PageJson(page,end-begin,cursor,false));
        }
        if( method == "resource.io_history" )
        {
            std::unordered_set<uint64_t> ioIds;
            for(const auto* v:activeRelations) if(v->targetKind==10&&isReachable(v->sourceKind,v->sourceId)) ioIds.emplace(v->targetId);
            for(const auto& v:contexts) if(v.timeNs<=effectiveTimeNs&&isReachable(8,v.contextId)&&v.relatedKind==10) ioIds.emplace(v.relatedId);
            json requests=json::array(); for(const auto& v:source->GetIoRequests()) if(ioIds.contains(v.requestId)) requests.push_back({
                {"ref",v.ref},{"request_id",Decimal(v.requestId)},{"resource_id",Decimal(v.resourceId)},{"parent_id",Decimal(v.parentId)},
                {"queue_ns",Decimal(v.queueNs)},{"start_ns",v.startNs?json(Decimal(*v.startNs)):json(nullptr)},{"end_ns",v.endNs?json(Decimal(*v.endNs)):json(nullptr)},
                {"requested_bytes",Decimal(v.requestedBytes)},{"transferred_bytes",Decimal(v.transferredBytes)},{"status",v.status},{"orphan",v.orphan},{"truncated",v.truncated}});
            auto result=base(); result["complete"]=graphComplete; result["resource"]=selectedJson(); result["requests"]=std::move(requests); return Success(id,std::move(result),trace);
        }

        json nodes=json::array(), edges=json::array();
        const size_t maxNodes=std::min<size_t>(params.value("max_nodes",size_t(1024)),4096), maxEdges=std::min<size_t>(params.value("max_edges",size_t(2048)),8192);
        auto evidenceRef=[](uint8_t kind,uint64_t value){
            static constexpr const char* prefixes[]={"resource:","asset:","object:","native-root:","gfx-resource:","logical-resource:",
                "physical-allocation:","resource-part:","resource-context:","gpu-pass:","io-request:","job:","shared-owner:","frame:"};
            return std::string(kind<sizeof(prefixes)/sizeof(prefixes[0])?prefixes[kind]:"resource:")+std::to_string(value);
        };
        std::set<std::pair<uint8_t,uint64_t>> evidenceKeys=reachableKeys;
        bool evidenceTraversalTruncated=false;
        if( selectedKind==5 || selectedKind==6 )
        {
            std::vector<std::pair<uint8_t,uint64_t>> reverseFrontier;
            for( const auto logicalId : logicalIds ) if( evidenceKeys.emplace(5,logicalId).second ) reverseFrontier.emplace_back(5,logicalId);
            if( selectedKind==5 && reverseFrontier.empty() ) reverseFrontier.emplace_back(5,selectedId);
            for( size_t cursor=0; cursor<reverseFrontier.size(); cursor++ )
            {
                if( evidenceKeys.size()>=maxNodes ) { evidenceTraversalTruncated=true; break; }
                const auto current=reverseFrontier[cursor];
                if( const auto incoming=activeRelationsByTarget.find(current); incoming!=activeRelationsByTarget.end() )
                    for( const auto* edge : incoming->second )
                    {
                        const std::pair<uint8_t,uint64_t> sourceKey { edge->sourceKind, edge->sourceId };
                        if( evidenceKeys.emplace(sourceKey).second ) reverseFrontier.emplace_back(sourceKey);
                    }
                if( current.first==4 ) if( const auto ranges=activeRangesByTarget.find(current); ranges!=activeRangesByTarget.end() )
                    for( const auto* range : ranges->second )
                    {
                        const std::pair<uint8_t,uint64_t> partKey { 7,range->partId };
                        if( evidenceKeys.emplace(partKey).second ) reverseFrontier.emplace_back(partKey);
                    }
                if( current.first==5 ) for( const auto& binding : gfx )
                {
                    if( binding.timeNs>effectiveTimeNs || binding.eventKind!=1 || binding.targetKind!=5 || binding.targetId!=current.second ) continue;
                    const auto active=activeGfxEpoch.find(binding.gfxResourceId);
                    if( active==activeGfxEpoch.end() || active->second!=binding.generation ) continue;
                    const std::pair<uint8_t,uint64_t> gfxKey { 4,binding.gfxResourceId };
                    if( evidenceKeys.emplace(gfxKey).second ) reverseFrontier.emplace_back(gfxKey);
                }
            }
        }
        std::set<std::pair<uint8_t,uint64_t>> emittedNodeKeys;
        auto addEvidenceNode=[&](uint8_t kind,uint64_t value){
            if(nodes.size()>=maxNodes||!emittedNodeKeys.emplace(kind,value).second)return;
            nodes.push_back({{"kind",kind},{"id",Decimal(value)},{"ref",evidenceRef(kind,value)}});
        };
        for(const auto& key:evidenceKeys) addEvidenceNode(key.first,key.second);
        for(const auto logicalId:logicalIds)
        {
            addEvidenceNode(5,logicalId);
            for(const auto& value:gpu->logicalResources) if(value.logicalResourceId==logicalId&&value.physicalAllocationId) addEvidenceNode(6,value.physicalAllocationId);
        }
        if(selectedKind==9||selectedKind==13) for(const auto& pass:gpu->passes)
            if((selectedKind==9&&pass.passId==selectedId)||(selectedKind==13&&pass.frame==selectedId)) addEvidenceNode(9,pass.passId);
        auto isEvidenceReachable=[&](uint8_t kind,uint64_t value){return evidenceKeys.contains(std::pair<uint8_t,uint64_t>{kind,value});};
        for(const auto* v:activeRelations) if(edges.size()<maxEdges&&isEvidenceReachable(v->sourceKind,v->sourceId)&&isEvidenceReachable(v->targetKind,v->targetId)) edges.push_back({
            {"source_kind",v->sourceKind},{"source_id",Decimal(v->sourceId)},{"target_kind",v->targetKind},{"target_id",Decimal(v->targetId)},
            {"relation",v->relation},{"relation_revision",v->revision},{"source_epoch",v->sourceEpoch},{"target_epoch",v->targetEpoch},
            {"provenance",uint8_t(v->provenance&0x0F)},{"derived",false}});
        for(const auto& v:gpu->logicalResources) if(edges.size()<maxEdges&&logicalIds.contains(v.logicalResourceId)&&v.physicalAllocationId) edges.push_back({
            {"source_kind",5},{"source_id",Decimal(v.logicalResourceId)},{"target_kind",6},{"target_id",Decimal(v.physicalAllocationId)},
            {"relation","logical_backed_by_physical"},{"provenance","derived-exact-generation"},{"derived",true}});
        for(const auto* v:activeRanges) if(edges.size()<maxEdges&&isEvidenceReachable(7,v->partId)&&isEvidenceReachable(v->targetKind,v->targetId)) edges.push_back({
            {"source_kind",7},{"source_id",Decimal(v->partId)},{"target_kind",v->targetKind},{"target_id",Decimal(v->targetId)},
            {"relation","part_maps_gfx_range"},{"relation_revision",v->revision},{"target_epoch",v->generation},
            {"byte_offset",Decimal(uint64_t(v->byteOffset))},{"byte_length",Decimal(uint64_t(v->byteLength))},
            {"provenance","exact-resource-range"},{"derived",false}});
        for( const auto& pass : gpu->passes ) if( edges.size()<maxEdges &&
            ( ( selectedKind == 9 && pass.passId == selectedId ) || ( selectedKind == 13 && pass.frame == selectedId ) || isReachable(9,pass.passId) ) )
        {
            if( selectedKind == 13 && edges.size()<maxEdges ) edges.push_back({
                {"source_kind",13},{"source_id",Decimal(selectedId)},{"target_kind",9},{"target_id",Decimal(pass.passId)},
                {"relation","frame_executes_pass"},{"provenance","exact-d3d12"},{"derived",false}});
            for( const auto& use : pass.uses ) if( edges.size()<maxEdges && logicalIds.contains(use.allocationId) ) edges.push_back({
                {"source_kind",9},{"source_id",Decimal(pass.passId)},{"target_kind",5},{"target_id",Decimal(use.allocationId)},
                {"relation","pass_uses_logical_resource"},{"provenance","exact-d3d12"},{"derived",false},
                {"usage_mask",use.usageMask},{"resource_set_id",use.resourceSetId}});
        }
        auto result=base(); result["complete"]=graphComplete&&gpu->complete&&!evidenceTraversalTruncated&&edges.size()<maxEdges;
        result["resource"]=selectedJson(); result["nodes"]=std::move(nodes); result["edges"]=std::move(edges);
        result["traversal_direction"]=(selectedKind==5||selectedKind==6)?"reverse_to_origin":"forward_to_usage";
        result["truncated"]=evidenceTraversalTruncated||edges.size()>=maxEdges; result["quality"]={{"resource_graph_complete",graphComplete},{"gpu_attribution_complete",gpu->complete},{"dropped",Decimal(dropped)}};
        return Success(id,std::move(result),trace);
    }
