...
try
{
    ...
    Assert-Condition ([string]$inspectTool.outputSchema.properties.schema_version.const -eq '1.15.0') 'MCP output schema version mismatch'

    $schema = Inspect '' 'system.schema'
    $describe = Inspect '' 'system.describe'
    Assert-Condition ([string]$schema.schema_version -eq '1.15.0') 'Query schema version mismatch'
