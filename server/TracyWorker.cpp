...
#pragma pack( pop )
        ...
        if( schemaVersion >= 9 )
        {
            ReadJnVector( f, jn.assets, "asset" );
            ReadJnVector( f, jn.assetUpdates, "asset update" );
            ReadJnVector( f, jn.unityObjectCreates, "UnityObject create" );
            ReadJnVector( f, jn.unityObjectUpdates, "UnityObject update" );
            ReadJnVector( f, jn.unityObjectDestroys, "UnityObject destroy" );
            ReadJnVector( f, jn.nativeRoots, "native root" );
            ReadJnVector( f, jn.gfxResourceBinds, "Gfx resource bind" );
            ReadJnVector( f, jn.gfxResourceUnbinds, "Gfx resource unbind" );
            ReadJnVector( f, jn.resourceParts, "resource part" );
            if( schemaVersion >= 10 )
            {
                ReadJnVector( f, jn.resourceRanges, "resource range" );
            }
            else
            {
                // ResourceGraph v1 was never released, but its raw vector has
                // to be consumed with the old element size so the following JN
                // domains remain readable.  The records are deliberately not
                // promoted into the v2 graph.
                struct LegacyJnResourceRangeData
                {
                    uint64_t partId;
                    uint64_t targetId;
                    uint32_t byteOffset;
                ...
                static_assert( sizeof( LegacyJnResourceRangeData ) == 32 );
                std::vector<LegacyJnResourceRangeData> unsupportedRanges;
                ReadJnVector( f, unsupportedRanges, "legacy resource range" );
            }
            ReadJnVector( f, jn.resourceContexts, "resource context" );
            ReadJnVector( f, jn.resourceRelations, "resource relation" );
            ReadJnVector( f, jn.resourceMetadata, "resource metadata" );
            ReadJnVector( f, jn.bootstrapStates, "resource bootstrap state" );
            ReadJnVector( f, jn.resourceGraphQuality, "resource graph quality" );
        }
    }
...
bool Worker::ProcessRecorder( const QueueItem& ev )
{
    ...
    case QueueType::JnAssetDefine:
    case QueueType::JnResourcePartDefine:
    case QueueType::JnResourceRange:
    case QueueType::JnResourceContext:
    case QueueType::JnUnityObjectCreate:
    case QueueType::JnUnityObjectDestroy:
    case QueueType::JnNativeRootDefine:
    case QueueType::JnGfxResourceBind:
    case QueueType::JnGfxResourceUnbind:
    case QueueType::JnResourceRelation:
    case QueueType::JnBootstrapState:
    case QueueType::JnResourceGraphQuality:
        RecorderCheckCurrentThread();
        break;
    case QueueType::JnAssetUpdate:
        if( ev.jnAssetUpdate.field == uint8_t( JnAssetField::Name ) ||
            ev.jnAssetUpdate.field == uint8_t( JnAssetField::SubAssetName ) ||
            ev.jnAssetUpdate.field == uint8_t( JnAssetField::Path ) ||
            ev.jnAssetUpdate.field == uint8_t( JnAssetField::BundleIdentity ) ) CheckString( ev.jnAssetUpdate.value );
        RecorderCheckCurrentThread();
        break;
    case QueueType::JnResourceMetadata:
        if( ev.jnResourceMetadata.valueKind == uint8_t( JnResourceMetadataValueKind::String ) )
            CheckString( ev.jnResourceMetadata.value );
        RecorderCheckCurrentThread();
        break;
    case QueueType::JnUnityObjectUpdate:
        if( ( ev.jnUnityObjectUpdate.fieldMask & ( uint16_t( JnUnityObjectField::Name ) |
            uint16_t( JnUnityObjectField::TypeName ) ) ) != 0 ) CheckString( ev.jnUnityObjectUpdate.value );
        RecorderCheckCurrentThread();
        break;
...
bool Worker::Process( const QueueItem& ev )
{
    ...
    case QueueType::JnCallsiteDefinition:
        ...
        break;
    case QueueType::JnAssetDefine:
        ProcessJnAssetDefine( ev.jnAssetDefine );
        break;
    case QueueType::JnAssetUpdate:
        ProcessJnAssetUpdate( ev.jnAssetUpdate );
        break;
    case QueueType::JnUnityObjectCreate:
        ProcessJnUnityObjectCreate( ev.jnUnityObjectCreate );
        break;
    case QueueType::JnUnityObjectUpdate:
        ProcessJnUnityObjectUpdate( ev.jnUnityObjectUpdate );
        break;
    case QueueType::JnUnityObjectDestroy:
        ProcessJnUnityObjectDestroy( ev.jnUnityObjectDestroy );
        break;
    case QueueType::JnNativeRootDefine:
        ProcessJnNativeRootDefine( ev.jnNativeRootDefine );
        break;
    case QueueType::JnGfxResourceBind:
        ProcessJnGfxResourceBind( ev.jnGfxResourceBind );
        break;
    case QueueType::JnGfxResourceUnbind:
        ProcessJnGfxResourceUnbind( ev.jnGfxResourceUnbind );
        break;
    case QueueType::JnResourcePartDefine:
        ProcessJnResourcePartDefine( ev.jnResourcePartDefine );
        break;
    case QueueType::JnResourceRange:
        ProcessJnResourceRange( ev.jnResourceRange );
        break;
    case QueueType::JnResourceContext:
        ProcessJnResourceContext( ev.jnResourceContext );
        break;
    case QueueType::JnResourceRelation:
        ProcessJnResourceRelation( ev.jnResourceRelation );
        break;
    case QueueType::JnResourceMetadata:
        ProcessJnResourceMetadata( ev.jnResourceMetadata );
        break;
    case QueueType::JnBootstrapState:
        ProcessJnBootstrapState( ev.jnBootstrapState );
        break;
    case QueueType::JnResourceGraphQuality:
        ProcessJnResourceGraphQuality( ev.jnResourceGraphQuality );
        break;
    default:
        assert( false );
        break;
...
void Worker::ProcessJnAssetDefine( const QueueJnAssetDefine& ev )
{
    auto& data = m_data.jnTrace;
    data.present = true;
    data.schemaVersion = JnTraceSchemaVersion;
    data.assets.push_back( JnAssetData { ev.assetEntityId, ev.assetIdHigh, ev.assetIdLow, m_threadCtx,
        ev.identityNamespace, ev.assetKind, ev.flags } );
}

void Worker::ProcessJnAssetUpdate( const QueueJnAssetUpdate& ev )
{
    if( ev.field == uint8_t( JnAssetField::Name ) || ev.field == uint8_t( JnAssetField::SubAssetName ) ||
        ev.field == uint8_t( JnAssetField::Path ) || ev.field == uint8_t( JnAssetField::BundleIdentity ) )
        CheckString( ev.value );
    const auto time = TscTime( ev.time );
    auto& data = m_data.jnTrace;
    data.present = true;
    data.schemaVersion = JnTraceSchemaVersion;
    data.assetUpdates.push_back( JnAssetUpdateData { time, ev.assetEntityId, ev.value, m_threadCtx,
        ev.revision, ev.field, ev.flags } );
    if( m_data.lastTime < time ) m_data.lastTime = time;
}

void Worker::ProcessJnUnityObjectCreate( const QueueJnUnityObjectCreate& ev )
{
    const auto time = TscTime( ev.time );
    auto& data = m_data.jnTrace;
    data.present = true;
    data.schemaVersion = JnTraceSchemaVersion;
    data.unityObjectCreates.push_back( JnUnityObjectCreateData { time, ev.objectId, ev.nativePointer,
        m_threadCtx, ev.instanceId, ev.runtimeTypeIndex, ev.flags } );
    if( m_data.lastTime < time ) m_data.lastTime = time;
}

void Worker::ProcessJnUnityObjectUpdate( const QueueJnUnityObjectUpdate& ev )
{
    if( ( ev.fieldMask & ( uint16_t( JnUnityObjectField::Name ) |
        uint16_t( JnUnityObjectField::TypeName ) ) ) != 0 ) CheckString( ev.value );
    const auto time = TscTime( ev.time );
    auto& data = m_data.jnTrace;
    data.present = true;
    data.schemaVersion = JnTraceSchemaVersion;
    data.unityObjectUpdates.push_back( JnUnityObjectUpdateData { time, ev.objectId, ev.value, m_threadCtx,
        ev.revision, ev.fieldMask, ev.flags } );
    if( m_data.lastTime < time ) m_data.lastTime = time;
}

void Worker::ProcessJnUnityObjectDestroy( const QueueJnUnityObjectDestroy& ev )
{
    const auto time = TscTime( ev.time );
    auto& data = m_data.jnTrace;
    data.present = true;
    data.schemaVersion = JnTraceSchemaVersion;
    data.unityObjectDestroys.push_back( JnUnityObjectDestroyData { time, ev.objectId, m_threadCtx, ev.flags } );
    if( m_data.lastTime < time ) m_data.lastTime = time;
}

void Worker::ProcessJnNativeRootDefine( const QueueJnNativeRootDefine& ev )
{
    const auto time = TscTime( ev.time );
    auto& data = m_data.jnTrace;
    data.present = true;
    data.schemaVersion = JnTraceSchemaVersion;
    data.nativeRoots.push_back( JnNativeRootData { time, ev.rootId, ev.objectId, m_threadCtx, ev.flags } );
    if( m_data.lastTime < time ) m_data.lastTime = time;
}

void Worker::ProcessJnGfxResourceBind( const QueueJnGfxResourceBind& ev )
{
    const auto time = TscTime( ev.time );
    auto& data = m_data.jnTrace;
    data.present = true;
    data.schemaVersion = JnTraceSchemaVersion;
    data.gfxResourceBinds.push_back( JnGfxResourceBindData { time, ev.gfxResourceId, ev.targetId,
        m_threadCtx, ev.generation, ev.targetKind, ev.resourceKind, ev.flags } );
    if( m_data.lastTime < time ) m_data.lastTime = time;
}

void Worker::ProcessJnGfxResourceUnbind( const QueueJnGfxResourceUnbind& ev )
{
    const auto time = TscTime( ev.time );
    auto& data = m_data.jnTrace;
    data.present = true;
    data.schemaVersion = JnTraceSchemaVersion;
    data.gfxResourceUnbinds.push_back( JnGfxResourceUnbindData { time, ev.gfxResourceId, m_threadCtx,
        ev.generation, ev.reason, ev.flags } );
    if( m_data.lastTime < time ) m_data.lastTime = time;
}

void Worker::ProcessJnResourcePartDefine( const QueueJnResourcePartDefine& ev )
{
    const auto time = TscTime( ev.time );
    auto& data = m_data.jnTrace;
    data.present = true;
    data.schemaVersion = JnTraceSchemaVersion;
    data.resourceParts.push_back( JnResourcePartData { time, ev.partId, ev.ownerId, m_threadCtx,
        ev.index, ev.semanticKind, ev.ownerKind, ev.flags } );
    if( m_data.lastTime < time ) m_data.lastTime = time;
}

void Worker::ProcessJnResourceRange( const QueueJnResourceRange& ev )
{
    auto& data = m_data.jnTrace;
    data.present = true;
    data.schemaVersion = JnTraceSchemaVersion;
    data.resourceRanges.push_back( JnResourceRangeData { 0, ev.partId, ev.targetId, ev.byteOffset,
        ev.byteLength, ev.generation, 0, ev.targetKind, ev.flags } );
}

void Worker::ProcessJnResourceContext( const QueueJnResourceContext& ev )
{
    const auto time = TscTime( ev.time );
    auto& data = m_data.jnTrace;
    data.present = true;
    data.schemaVersion = JnTraceSchemaVersion;
    data.resourceContexts.push_back( JnResourceContextData { time, ev.contextId, ev.relatedId, m_threadCtx,
        ev.callsiteId, ev.relatedKind, ev.stage, ev.flags } );
    if( m_data.lastTime < time ) m_data.lastTime = time;
}

void Worker::ProcessJnResourceRelation( const QueueJnResourceRelation& ev )
{
    const auto time = TscTime( ev.time );
    auto& data = m_data.jnTrace;
    data.present = true;
    data.schemaVersion = JnTraceSchemaVersion;
    uint32_t revision = ev.sequence;
    uint32_t sourceEpoch = 0;
    uint32_t targetEpoch = 0;
    const auto pending = m_jnPendingResourceRelations.find( m_threadCtx );
    if( pending != m_jnPendingResourceRelations.end() &&
        pending->second.sourceId == ev.sourceId && pending->second.sourceKind == ev.sourceKind &&
        uint16_t( pending->second.revision ) == ev.sequence )
    {
        revision = pending->second.revision;
        for( uint8_t index=0; index<pending->second.epochCount; index++ )
        {
            const auto& epoch = pending->second.epochs[index];
            if( sourceEpoch == 0 && epoch.entityId == ev.sourceId && epoch.entityKind == ev.sourceKind )
                sourceEpoch = epoch.value;
            else if( targetEpoch == 0 && epoch.entityId == ev.targetId && epoch.entityKind == ev.targetKind )
                targetEpoch = epoch.value;
        }
        m_jnPendingResourceRelations.erase( pending );
    }
    data.resourceRelations.push_back( JnResourceRelationData { time, ev.sourceId, ev.targetId, m_threadCtx,
        revision, sourceEpoch, targetEpoch, ev.sequence, ev.sourceKind, ev.targetKind, ev.relation, ev.provenance, ev.flags } );
    if( m_data.lastTime < time ) m_data.lastTime = time;
}

void Worker::ProcessJnResourceMetadata( const QueueJnResourceMetadata& ev )
{
    if( ev.valueKind == uint8_t( JnResourceMetadataValueKind::String ) ) CheckString( ev.value );
    const auto time = TscTime( ev.time );
    auto& data = m_data.jnTrace;
    data.present = true;
    data.schemaVersion = JnTraceSchemaVersion;
    if( ev.key == 8 )
    {
        auto& pending = m_jnPendingResourceRelations[m_threadCtx];
        pending = {};
        pending.sourceId = ev.entityId;
        pending.sourceKind = ev.entityKind;
        pending.revision = uint32_t( ev.value );
    }
    else if( ev.key == 7 )
    {
        const auto pending = m_jnPendingResourceRelations.find( m_threadCtx );
        if( pending != m_jnPendingResourceRelations.end() && pending->second.revision == ev.revision &&
            pending->second.epochCount < 2 )
        {
            auto& epoch = pending->second.epochs[pending->second.epochCount++];
            epoch.entityId = ev.entityId;
            epoch.entityKind = ev.entityKind;
            epoch.value = uint32_t( ev.value );
        }
    }
    // QueueJnResourceRange uses the complete 31-byte payload and therefore
    // carries its revision/timestamp in the immediately following exact
    // RangeRevision metadata record. ResourceGraph serializes this triplet
    // under one registry mutex, so the newest unresolved matching Part range
    // is the unique record being annotated here.
    if( ev.key == 9 && ev.entityKind == 7 )
    {
        for( auto it = data.resourceRanges.rbegin(); it != data.resourceRanges.rend(); ++it )
        {
            if( it->partId == ev.entityId && it->revision == 0 )
            {
                it->time = time;
                it->revision = uint32_t( ev.value );
                break;
            }
        }
    }
    data.resourceMetadata.push_back( JnResourceMetadataData { time, ev.entityId, ev.value, m_threadCtx,
        ev.revision, ev.key, ev.entityKind, ev.valueKind } );
    if( m_data.lastTime < time ) m_data.lastTime = time;
}

void Worker::ProcessJnBootstrapState( const QueueJnBootstrapState& ev )
{
    const auto time = TscTime( ev.time );
    auto& data = m_data.jnTrace;
    data.present = true;
    data.schemaVersion = JnTraceSchemaVersion;
    data.bootstrapStates.push_back( JnBootstrapStateData { time, ev.highWatermark, m_threadCtx,
        ev.connectionGeneration, ev.emittedCount, ev.remainingCount, ev.phase, ev.flags } );
    if( m_data.lastTime < time ) m_data.lastTime = time;
}

void Worker::ProcessJnResourceGraphQuality( const QueueJnResourceGraphQuality& ev )
{
    const auto time = TscTime( ev.time );
    auto& data = m_data.jnTrace;
    data.present = true;
    data.schemaVersion = JnTraceSchemaVersion;
    data.resourceGraphQuality.push_back( JnResourceGraphQualityData { time, ev.relatedId, ev.value,
        m_threadCtx, ev.counter, ev.reason, ev.flags } );
    if( m_data.lastTime < time ) m_data.lastTime = time;
}
...
void Worker::Write( FileWrite& f, bool fiDict )
{
    ...
    WriteJnVector( f, m_data.jnTrace.assets );
    WriteJnVector( f, m_data.jnTrace.assetUpdates );
    WriteJnVector( f, m_data.jnTrace.unityObjectCreates );
    WriteJnVector( f, m_data.jnTrace.unityObjectUpdates );
    WriteJnVector( f, m_data.jnTrace.unityObjectDestroys );
    WriteJnVector( f, m_data.jnTrace.nativeRoots );
    WriteJnVector( f, m_data.jnTrace.gfxResourceBinds );
    WriteJnVector( f, m_data.jnTrace.gfxResourceUnbinds );
    WriteJnVector( f, m_data.jnTrace.resourceParts );
    WriteJnVector( f, m_data.jnTrace.resourceRanges );
    WriteJnVector( f, m_data.jnTrace.resourceContexts );
    WriteJnVector( f, m_data.jnTrace.resourceRelations );
    WriteJnVector( f, m_data.jnTrace.resourceMetadata );
    WriteJnVector( f, m_data.jnTrace.bootstrapStates );
    WriteJnVector( f, m_data.jnTrace.resourceGraphQuality );
}
